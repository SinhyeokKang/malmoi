...
Crowdin/Tolgee 대체가 목표가 아니라 학습·실험이다. **MVP(PoC)는 2026-09-05에 닫혔고 지금은 SaaS화 단계다** — 인증·인가(2단계, 2026-09-06 프로덕션 반영), GitHub 설치 연결(4단계)과 탐지 온보딩(5단계)이 **2026-09-07에 프로덕션까지** 갔다(PR [#9](https://github.com/SinhyeokKang/malmoi/pull/9) → `f595cc3`). **6단계(번역 UI 재작성 + Publish)에 들어갔다** — 6a를 4번의 배송으로 쪼갰고 ship 1(기반) · ship 2(셸) · ship 3(번역 화면 + Publish)이 PR [#12](https://github.com/SinhyeokKang/malmoi/pull/12) → `46df51a` · PR [#14](https://github.com/SinhyeokKang/malmoi/pull/14) → `add099a` · PR [#15](https://github.com/SinhyeokKang/malmoi/pull/15) → `ef9da44`로 프로덕션, ship 4(설정·새 프로젝트·초대 수락 + 문서·chore)가 dev에 있다. 남은 것은 6b 넷이다. 정본은 [docs/SAAS.md](./docs/SAAS.md).
...
Next.js 16 App Router · Supabase Postgres + Prisma 7 · Auth.js v5 DB 세션 (GitHub·Google 로그인 — 인가는 `ProjectMember`) · GitHub App · Tailwind 4 + **자체 프리미티브**(`components/ui/` 16개, Radix 3종) + 앱 셸 + `messages/en.tsx` 단일 사전(**UI는 영어 단일**, `lang="en"`) (**라이트 단일, `dark:` 금지**) · Vercel
