...
**확인 callback에서는 기존 브라우저 세션을 Auth.js에 넘기지 않는다** (2026-09-12 리뷰 수정).
`withLoginLink`가 요청 복사본의 세션 쿠키만 제외해 새 OAuth로 확인한 사용자로 로그인시킨다.
다른 사용자 세션이 남아 있으면 Auth.js가 **연결 커밋 뒤에** `OAuthAccountNotLinked`를 던지기
때문이다. 실패 시 브라우저의 기존 쿠키는 유지되고, 성공 시 새 세션 쿠키로 교체된다.
**연결 커밋과 세션 생성은 별개다** — 세션 생성이 실패해도 Account 추가는 남는다. 래퍼는 이를
성공 착지로 덮지 않고 `/signin?error=Unavailable`로 보내며, 다음 일반 로그인으로 복구한다.
이미 소비된 challenge로 돌아가면 `LinkExpired`가 장애 사유를 지우므로 그 화면으로 보내지 않는다.
...
⚠️ **노출 최소화** — ⚠️ **2026-09-12에 한 칸 완화됐다**(사용자, 실물 확인 중). 보이는 것은
**마스킹한 이메일 · provider 이름 · 가입 월 · 표시 이름 · 아바타 이미지**이고, **프로젝트 수·멤버
목록 같은 계정 내용은 계속 싣지 않는다.** 서버가 `loadChallengeView`로 **보일 값만** 만들어
내려준다(`loadMembers`가 원문 이메일을 안 돌려주는 것과 같은 규칙 — 클라이언트에서 가리면 원문이
이미 RSC 페이로드에 있다).

⚠️ **완화한 근거**: 이 화면에 도달하려면 요청자가 **이미 그 주소를 IdP에서 검증받았어야 한다**(§0)
— 이름을 가려도 그 사람은 이미 주소를 안다. 반면 가리는 대가는 실물에서 바로 드러났다: `Avatar`가
1행 문자열에서 이니셜을 뽑으므로 **마스킹한 주소의 첫 글자**가 아바타가 되어(`o***@…` → `o`) 셸의
아바타(표시 이름 → `s`)와 **다른 글자·다른 색**이 됐고, "이게 내 계정인가"를 묻는 카드가 남의
계정처럼 보였다. 그래서 **1행은 마스킹한 이메일 그대로**이고 **아바타만** 이름·이미지에서 온다
(`EntityCard`의 `avatarName`·`image`).

⚠️ **남는 노출은 훔친 challenge URL이다** — 그 URL을 얻은 사람이 이름·아바타를 본다(⑧이 그
시나리오를 상정한다). 붙이려면 여전히 **기존 provider의 OAuth를 통과**해야 하므로 계정이
열리지는 않는다.
