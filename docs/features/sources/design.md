...
⚠️ **"좌우 48"의 정체는 A0에서 확인한다** — 껍데기 머리 여백은 `px-8`(32)이고(`modal.tsx:173`)
48은 dim padding(`w-[calc(100%-96px)]`)이다. 시안의 본문 좌우가 48이면 그것은 껍데기 값 32와의 **이탈**이고
§6.646의 *"껍데기 값을 바꾸면 온보딩 네 단계가 함께 움직인다"*가 걸린다.
⚠️ **목록 상태의 배지 사용 여부도 A0에서 확인한다** — 현행 카드는 평문 두 색이고
(`sources-card.tsx:51` — `failed ? text-destructive : text-muted-foreground`) DESIGN §6.2가
연결 건강성 7종에서 *"배지를 쓰지 않는다"*를, 목록 행 5종에서 그 예외를 각각 등재했다.
