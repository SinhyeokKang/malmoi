...
- [ ] 버튼 배경 — 시안 `#090b0c` vs `bg-primary`(남색 틴트)
- [ ] 보조 텍스트 — 시안 `#6f6f6f` vs `text-muted-foreground`
- [ ] 링크 — 시안 `#537bff` vs `blue-600`
      - ⚠️ **`blue-500`은 리포 전수 0건의 미등재 색이다** (CDO 🔴4) — 쓰면 §6.2 등재가 필요하다
- [ ] 도트 색 — 시안 `#006BFF` @5%
- [ ] 우측 배경 그라데이션 — 시안 값 미확보
- [ ] **KV 세로 보정값** — 그림자 여백 때문에 위로 치우친다
- [ ] **버튼 radius** — base `rounded-md`(8px) vs `rounded-xl`(14px), 시안은 12px
- [ ] **패널 border** — `--border`(`#e2e8f0`)가 시안(`#f5f6f7`)보다 한 단계 진하다. 선이 도드라지면
      `--border-subtle` 신설(리포의 시맨틱 border 토큰이 `--border` 하나뿐이라 지금은 선택지가 없다)
- [ ] **바깥 배경·hero 그라데이션** — `--auth-canvas` · `--auth-hero-from/to`. "좀 더 강하게"를 반영했고
      실물 판단이 남았다
- [ ] **도트 알파** — 기본 0.08(시안 5%보다 한 단계) · 커서 중심 0.3
...
| 항목 | 지금 |
|---|---|
| KV 세로 보정값 | 그림자 여백이 비대칭이라 목측 (T11) |
| 우측 배경 그라데이션 | 시안 값 미확보 → T11 |
| 초대 화면의 우측 패널 | KV에 환영 문구가 구워져 있어 실패 분기와 어긋난다 (T10) |
| `Button` 기본 치수 교체 시점 | 마지막 화면이 옮겨온 뒤 |
| **토스트가 진입 시점 상태에 맞나** | 2026-09-08이 거절한 방향이고 `role="alert"` 강등이 딸린다. **T11 스크린리더 확인이 판정**이다 |
