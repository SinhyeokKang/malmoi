...
| `malmoi-kv-1.png` | 1296×900 (3x) | 프로젝트 카드 | 우측 패널 |
| `malmoi-kv-2.png`·`malmoi-kv-3.png`·`malmoi-kv-4.png` | 각각 648×606 (3x) | 한국어·영어·일본어 카드 | 우측 패널 |
...
**2026-09-12 사용자 결정: 합성 PNG를 그림자 없는 카드 PNG 네 장으로 교체한다.**
`next/image` + `priority`로 각 이미지를 최적화하고, 앞쪽 세 카드의 그림자는 기존 `shadow-low`·`shadow-medium`을 쓴다.

- 우측 패널은 사방 80px 패딩, KV 래퍼는 `w-full max-w-[768px]` + `aspect-[724/332]`다.
- 바깥 패딩 8px·컬럼 gap 8px을 포함하면 KV 폭은 1280px 화면에서 468px, 1440px에서 548px,
  1920px에서 768px이다. 최소 레이아웃 폭 1280px 아래에서는 가로 스크롤한다.
- 기존 합성 이미지의 1x 좌표계를 유지한다: 프로젝트 `(146, 0, 432, 300)`, 번역 카드
  `(14, 114, 216, 202)`·`(254, 114, 216, 202)`·`(494, 114, 216, 202)`.
  크기·좌표를 래퍼 대비 비율로 배치하므로 네 카드가 함께 축소되고 기존 그림자 여백도 유지된다.
- 앞쪽 번역 카드는 각각 hover 시 8px 위로 이동하고 그림자를 `low`에서 `medium`으로 바꾼다.
  전환은 300ms ease-out이며, `prefers-reduced-motion`에서는 이동·전환을 끈다.
  고정된 hover 래퍼가 움직이는 이미지를 감싸 하단에서 포인터가 빠졌다 들어오는 떨림을 막는다.
- 프로젝트 카드는 그림자 없이 고정한다. 장식에 버튼 역할·키보드 탭 정지를 추가하지 않는다.
