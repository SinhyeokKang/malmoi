...
| 2026-09-12 | **shadcn `<table>` 전환 + 조회 순서** | **0.30~0.32초** | **0.29~0.32초** | ✅ 게이트 통과. ⚠️ **축이 위와 다르다 — 로컬 프로덕션 빌드다**(`AUTH_TRUST_HOST=true pnpm start` → dev DB). 네트워크가 0이므로 preview 숫자와 직접 비교하지 않는다 |

⚠️ **2026-09-12 회차는 FCP 밖을 함께 쟀다.** 게이트가 FCP인데 이 화면의 비용은 그 뒤에 있다:

| | 행 | 문서 | DOM 노드 | FCP | DOMContentLoaded | **loadEventEnd** |
|---|---|---|---|---|---|---|
| 기본 착지(`?ns=actionLog`) | 93 | 57KB | 1,513 | 0.30~0.32초 | 0.70~0.80초 | 0.87~1.00초 |
| `?ns=*` | **2,721** | **1.44MB** | **34,624** | 0.29~0.32초 | 1.34~1.47초 | **3.89~3.92초** |

- **FCP가 행 수와 거의 무관하다** — 스트리밍이라 첫 페인트는 셸에서 난다. 게이트가 재는 것이 그것이고, 통과는 진짜다.
- ⚠️ **`loadEventEnd`는 3.9초다** — `<Textarea>` 2,721개와 노드 34,624개의 하이드레이션 비용이고 **FCP 게이트가 원리적으로 못 보는 축**이다. 로컬이라 네트워크가 0인데도 그렇고, preview에서는 1.44MB 전송이 더해진다.
- **그래도 지금 가상화를 넣지 않는다** — T12의 판정 조건은 "게이트 미달"이고 게이트는 FCP다(SAAS §8 원문). `?ns=*`는 **기본 경로가 아니라** 사용자가 일부러 고르는 전체 보기이고, 기본 착지는 1초 안에 끝난다. 다음에 이 화면이 느리다는 제보가 오면 **FCP가 아니라 이 표의 `loadEventEnd`부터** 본다.
- 측정용 프로젝트는 dev DB의 `perf-903`이다(`SinhyeokKang/bugshot-2` · ts-dict · **907키** × 3로케일 — 수동 지정 경로로 만들었다: 자동 탐지는 `_locales`(4키)만 찾는다). 다음 회차가 같은 프로젝트를 쓴다.
