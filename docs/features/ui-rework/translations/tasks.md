...
- [x] **어느 로케일 코드에 어느 국기를 붙일지의 첫 목록**도 함께 받는다 — `en → gb`는 알고리즘이
      ...
      - ⚠️ **언어 표는 다섯 그대로다**(ko·en·ja·zh·fr) — 하위태그 없는 `de`·`es`·`pt`는 코드만으로
        선다. 넓힐지는 결정이 필요하다(`es`→ES/MX, `pt`→PT/BR처럼 **틀린 국기가 없는 것보다 나쁜**
        자리가 있다)
...
| 2026-09-11 | **행 축 (8-4)** | ⬜ 미측정 | ⬜ 미측정 | dev DB에 903키 프로젝트가 없다 — `push:local` 적재가 선행이다 |

⚠️ **미측정인 채로 dev에 나갔다.** 근거는 **재는 자가 행 수가 아니라 노드 수**라는 것이다 —
`<Textarea>`는 2,709개로 **그대로**이고(축이 바뀌어도 셀 수는 안 변한다), 늘어난 것은 행 래퍼와
로케일 배지이며 **국기는 CSS `background-image`라 요소가 0개 는다.** 그래도 **게이트는 게이트다**:
프로덕션(`/merge`) 전에 재고, 2초 미달이면 **그 자리에서 가상화를 판정한다**.
