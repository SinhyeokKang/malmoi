...
## T5 — 실물 확인 ✅ **9/11 통과** (2026-09-06 일곱 · 2026-09-07 철회 하나 + 프로덕션 왕복 하나. 둘은 설치 모양 때문에 불가)
...
- [x] **프로덕션 왕복** (2026-09-07, `5933524` 배포 뒤 — 위는 전부 로컬이었다). `https://mal-moi.com`에서
      "GitHub 연결" → `redirect_uri=https://mal-moi.com/api/github/callback`으로 GitHub 계정 선택
      화면(계정 둘 중 설치 소유자 `SinhyeokKang`) → 동의 화면 없이 되돌아와 `@SinhyeokKang`. 프로덕션
      DB에 `github-app` 행이 처음 생겼다. **`skillflo`(설치 목록에 일부러 없는 리포)에서 "연결"** →
      "이 리포에 App이 설치돼 있지 않아요" 인라인 — 사용자 토큰 GET이 프로덕션에서 통했고 3중 검증의
      첫 거부가 값으로 나온 것. **재연결 성공 경로**(`connectRepository` → `Project.update`)는 같은
      설치 id로 다시 넣으면 health가 곧 "연결됨"이 돼 버튼이 사라지므로 이렇게 밟았다: 리포를 뺀 상태의
      설정 화면을 탭에 둔 채 다른 탭에서 GitHub 선택 목록에 다시 넣고, **리로드 없이** 그 "다시 연결"을
      눌렀다 → 섹션이 "연결됨"으로 바뀜(`revalidatePath`). 뺀 상태에서 누르면 "설치돼 있지 않아요"
