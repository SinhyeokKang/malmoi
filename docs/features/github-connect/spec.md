...
> 🔴 **STALE — "같은 페이지에 얹는다"가 틀렸다** (2026-09-09, 6b-2가 구현하며 뒤집혔다). 멤버 관리는
> **별도 라우트 `/projects/:slug/members`** 다. 근거: 이 페이지는 `project:settings` 뒤라 **EDITOR가 아예
> 못 들어온다**(`/projects?e=forbidden` — 실측). 그런데 user-stories §5가 "EDITOR는 목록만 본다"로
> 결정했고 "누가 이 프로젝트에 있나"는 번역자에게도 필요한 정보다. 섹션으로 얹으면 그 스토리를
> 만족시킬 방법이 **게이트를 `translation:write`로 낮추는 것**뿐인데, 그러면 리포 연결·push 토큰이
> EDITOR에게 열린다. 게이트가 다른 두 화면은 라우트가 둘이어야 한다 — 정본은
> `features/translation-ui/design.md` §3.9다.
...
| 멤버 관리 섹션 | ~~6단계 — 같은 `/settings` 페이지에 얹는다 (§1)~~ 🔴 **STALE** — 6b-2가 **별도 라우트** `/projects/:slug/members`로 만들었다(2026-09-09). EDITOR가 목록을 봐야 하고 `/settings`는 `project:settings` 뒤라 게이트가 갈린다 — §1의 🔴 참고 |
