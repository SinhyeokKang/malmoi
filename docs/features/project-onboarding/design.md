...
- ⚠️ **`maxDuration = 60`은 그대로 둔다.** 실제로 편집이 쌓인 프로젝트가 여럿이면 넘칠 수 있고,
  ...
  가능한가"를 세지 않는다 — `lastPulledAt`은 **프로젝트마다** 성공 직후 쓰이므로 중간에 죽어도
  거기까지의 결과는 남고 편집을 잃지 않는다.
  - ⚠️ **"넘치면 나머지가 다음 밤에 돈다"는 거짓이다** (2026-09-07 code-review가 잡았다). 순서가
    `slug` 고정이라 선두 프로젝트가 예산을 독점하면 **뒤쪽 slug는 다음 밤에도 같은 순서로 밀려
    영영 도달되지 않는다**(결정적 아사). `lastPulledAt` 오름차순으로 돌리면 사라지지만 그건 이
    단계가 요청받지 않은 동작이고, 정렬을 바꾸면 "실패 지점을 재현한다"는 결정성의 근거도 다시
    설명해야 한다. **그래서 지금은 slug 고정을 유지하고 이 제약을 7단계 `SyncRun`에 넘긴다** —
    GitHub을 부르는 것은 편집이 쌓인 프로젝트뿐이라(1층 스킵) 실제로 밟으려면 그런 프로젝트가
    동시에 여럿이어야 한다.
- ⚠️ **실패는 두 갈래 모두 로그를 남긴다** (2026-09-07 구현에서 추가). 응답이 항상 200 배열이고
  cron은 본문을 버리므로, 로그가 없으면 **전 프로젝트가 매일 밤 실패해도 성공과 관측값이 같다**
  (POSTMORTEM 2026-09-06의 형태). `lib/pull/**`의 실패 대부분이 `fail()`(= 본문에 실리는 안전한
  메시지)이라 그쪽만 조용하면 정확히 그 상태가 된다. 순회 끝에 `[pull] targets=N failed=M` 한 줄을
  더 남긴다 — 전면 장애가 grep 하나로 잡혀야 한다.
