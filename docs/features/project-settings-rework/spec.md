...
6. **프로젝트에 이름표가 없다.** 생성 시 정한 `Project.name`을 **고칠 자리가 없고**, 이미지 컬럼
   자체가 없어 목록·Home·초대의 `ProjectThumbnail`이 항상 `toneOf(name)` 폴백으로만 선다
   (`src`를 받도록 이미 만들어져 있는데 그것을 주는 경로가 0이다).
...
- C1. `/projects/[slug]/settings`의 본문이 **카드 다섯**이다 — `General` · `Repository` ·
  `Translation sources` · `CI integration` · `Archive project`. `pnpm test`의 설정 화면 테스트가
  카드 제목 다섯을 순서대로 단언하고, `Status`·`GitHub account` 제목이 **없음**을 단언한다.
...
- C5. 썸네일을 올리면 그 이미지가 **설정 미리보기(56) · 프로젝트 목록 행(28) · Home 머리 · 초대 카드**
  넷에 전부 뜬다. [Remove]를 누르면 넷에서 함께 사라지고 Blob 객체가 지워진다.
...
- C7. 소스가 둘 이상인 프로젝트에서 **한 소스만** 적재에 실패하면, 실패한 그 행에만 사유가 서고
  나머지 행은 정상 상태를 보인다. 재시도 버튼은 **그 소스**를 대상으로 돈다.
