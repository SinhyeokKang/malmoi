...
push·pull·편집 UI 어디에도 붙지 않는다. **`/`의 표현층**만 바뀐다. `/docs`·`/privacy`는 이번 과업 밖이다(spec 비목표).
인가 모델은 그대로다 — `/`는 `entry-points.test.ts`의 `EXEMPT`이고 middleware matcher 밖이다.

## 구조

```
app/page.tsx                  rootView(status) → redirect | <Landing/>
components/landing/           shell(header · footer · scroller 잎) · hero · stage(클라이언트) · mockup/(씬 DOM, 서버·정적) · closing
lib/landing/stage.ts          순수 함수 (잎 — import는 타입뿐)
lib/auth/landing.ts           rootView (기존 파일)
lib/links.ts                  GitHub 리포 URL 상수 · 푸터 링크 목록(`/signin`과 공유)
```

- 셸은 소비자가 `/` 하나라 `components/landing/` 안에 둔다 — 공용 `public-shell/`로 빼는 것은 `/docs`·`/privacy`가 들어오는 후속 과업의 몫이다(선반영 금지).
...
- 헤더(시안 1a): 로고 32(`aria-label="Malmoi home"`) · `<nav aria-label="Main">` 링크 14/400(6/10 · radius 8) · hover `foreground` 알파 .03 ·
  **선택 상태를 그리지 않고 `aria-current="page"`만**(Home) · 우측 primary `md`(36) `Get started`. GitHub는 새 탭, 글리프 없음(§6.3).
  `Docs`는 기존 `/docs`(`routes.docs()`)로 간다.
- 푸터: 13 · muted · hover foreground. 링크 목록은 `/signin` 푸터와 **한 상수**에서 낸다(`GitHub · Privacy Policy · Docs`, `m.signIn.footer`의 값).
  GitHub URL 상수는 **`lib/routes.ts` 밖**(`lib/links.ts`)에 둔다(`auth-layout.tsx:64-67` — `entry-points.test.ts`가 외부 URL을 죽은 라우트로 잡는다).
...
- 새 raw 색 0(시안 1g) — 쓰는 값은 전부 토큰(`--canvas` · `--primary` · `--foreground` 알파 · `--border` · `--border-subtle` · `--muted-foreground` · 그림자 둘).
...
- **reduced-motion**: `s = fit`, `y = yPin` 고정, 베젤 0 · 고정 그림자 · 크롬 항상 보임. 씬은 `t ≥ 0.5 ? 1 : 0`(f = 0.8에서 단절). 스크롤 길이는 같다.
...
- `publicCta`(세션으로 갈리는 CTA)는 두지 않는다 — 랜딩은 `none`·`unavailable`에서만 그려지고 둘 다 `Get started`다. `/docs`·`/privacy` 헤더가 생기는 후속 과업에서 필요해진다.
- `lib/landing/stage.ts`는 클라이언트가 값으로 읽으므로 `client-graph.test.ts`의 `CLIENT_LIB_FILES`에 등재하고 잎 검사를 붙인다(`lib/protection/plan.ts` 형).
...
- 인증 경계: matcher·`EXEMPT` 불변. `/`의 `ok → /projects` 유지.
...
- 개인정보 방침: 새 목적·전송처·쿠키 0(폰트 자사 호스트, 외부 스크립트 없음). 방침 본문은 안 건드린다. `/push` 4단계에서 재확인만.
...
- **2026-09-19 (jsdom에 accname 없음)** — 목업 `aria-hidden`의 실측은 T8 CDP.
...
- DESIGN: 랜딩 절 신설(셸 좌표·스테이지 규칙·배율 표·목업 문구 경계), **§4에 weight 600 도입**(h1·CTA h2 — 첫 소비자). §6.61은 그대로.
- PRODUCT §7.7: `/` 행을 ✅ 랜딩으로, "로그아웃 → 랜딩" 문장 현재형으로. `PRODUCT.md:554-558`의 "지금 `/`는 … 껍데기" 문단.
- ARCHITECTURE: `:2071` 근처의 `/` 착지 판정 서술(`landingTarget` → `rootView`, `unavailable` → 랜딩).
- DIRECTORY: `components/landing/`·`lib/landing/`·`lib/links.ts`, `:11`의 "landingTarget이 정한 곳".
