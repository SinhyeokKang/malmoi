...
| 37 | 사용자 결정으로 제외 (2026-09-10) | repo admin 추가 요구는 구현하지 않음. 현재 가시성 기반 권한 정책 유지 |
| 38 | 후속 로컬 구현·자동 검증 | [session-revocation](../session-revocation/spec.md), 실제 OAuth/브라우저·배포 대기 |
...
아니므로 공격적 리포의 모든 파싱 비용을 보장하지 않는다. #37은 사용자 결정으로 구현 범위에서 제외했고, #38 전체 세션 회수는 후속 [session-revocation](../session-revocation/tasks.md)에서 로컬 구현·자동 검증했으며 실제 공급자·배포 게이트는 남는다.
