...
**사용자 결정 (2026-09-10):** #37은 구현하지 않는다. 최초 연결에 repo admin 권한을 추가 요구하지 않고 현행 접근 가능 정책을 유지한다. 위 권한 차이에 관한 감사 관측은 보존하며, 구현 대기 항목으로는 추적하지 않는다.
...
감사 당시 로그아웃은 현재 세션을 끝내며, 사용자가 자기 세션 전체를 회수할 기능은 없었다.
...
**후속 (2026-09-10):** [전체 세션 회수](../session-revocation/spec.md)를 credential 워크트리에서 로컬 구현·자동 검증했다. 같은 공급자 계정 확인 후 현재 기기를 포함해 삭제하며, 실제 공급자/브라우저 검증과 배포는 대기다. #37은 포함하지 않는다.
...
최초 감사 시점의 산출물은 이 보고서 하나였다. 앱 코드·DB·원격 설정 수정, 빌드·테스트,
staging·커밋·푸시는 수행하지 않았다.

### 후속 구현 상태 (2026-09-10)

감사 원문과 당시 관측은 위에 보존했다. 후속 코드 수정·테스트·검수·운영 잔여는
[작업 기록](./tasks.md)을 따른다. #29~35와 #40을 수정했고 #39는 신규 로그인 토큰 저장을 제거했다.
#36은 sync-runs 기준점에서 구현됐다. #37은 사용자 결정으로 구현 범위에서 제외했다. #38은 후속 로컬 구현·자동 검증 상태이며 실제 공급자/배포 검증과 기존 데이터의 credential 운영 전환은 남아 있다.
이 상태는 로컬 구현을 뜻하며 DB 적용·배포·실물 보안 검증 완료를 뜻하지 않는다.
...
후속 credential 워크트리에서 **로컬 구현과 격리 DB 검증을 진행했으며 운영 전환은 대기 중**이다. 39번의 신규 저장 제거에 더해 기존 토큰 정리 도구를 준비했고, 31번의 만료 세션 검사와
33번의 소유자 조건부 쓰기를 관련 경계에서 검증한다. 암호화만으로 OAuth 계정 연결 정책이나
다른 감사 발견이 해결된 것으로 취급하지 않는다.

실제 전환과 검증 경계는 [credential 운영 절차](../credential-storage/operations.md)를 따른다. #31·#33의 sec-audit 방어선은 credential 어댑터·암호문 CAS에도 유지하며, 기존 DB 전환 완료로는 표시하지 않는다.
