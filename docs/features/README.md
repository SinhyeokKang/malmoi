...
⚠️ **다음 단계 **둘**은 이름이 정해졌고 디렉터리만 없다** (SAAS §8): 7단계 `sync-runs/` · 8단계 `ui-rework/`(UI 재작성 — **시안 미확정이고 `/feature`는 7단계를 마친 뒤에 뜬다**). CLAUDE.md가
...
| [sec-audit](./sec-audit/) | 🚧 **ship 6개 + DB 트랙 전부 dev에 나갔다** (2026-09-09 하루 — 프로덕션 미반영) | ARCHITECTURE §1.1·§5.1·§5.5.05·§6.05·§8 · DESIGN §6.65 · CLAUDE.md(컨벤션·CI·게이트웨이·`lib/locale-code.ts`·`next.config.ts`) · ACTIONS.md · `/db` 5단계 · POSTMORTEM 4건(2026-09-09) | ⏳ **남은 것 넷이고 전부 `/merge` 뒤다**: ① **`l10n-push-v1` 태그를 `main` 커밋에 끊고 대상 리포 넷의 `@main`을 옮긴다**(발견 3은 그때 닫힌다 — 지금 문서·`renderWorkflowYaml`이 **실재하지 않는 ref**를 가리킨다) ② 온보딩 한 바퀴 실물(ship 5 T4) ③ 멤버 화면 페이로드 실물(ship 4 T9 — `/bugshot-qa` 자리) ④ 화면 넷이 CSP-RO에서 그대로 도는지(ship 6 T2). **수용 넷**(발견 7·16·27·28)과 **사람이 지울 `.env.local` 항목 셋**(발견 22)은 `tasks.md` 맨 아래에 있다 |
