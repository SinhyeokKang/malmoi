...
여섯째로 **`findings.md`**(우리가 돌린 감사의 발견 목록 — `sec-audit/`에만 있다)가 있다. 그 디렉터리는
**`spec.md`·`design.md`가 없는 유일한 축이다** — 설계할 기능이 아니라 이미 있는 코드에서 찾은 결함이라,
...
| [translation-ui](./translation-ui/) | ✅ **닫혔다** (SaaS **6a 닫힘** — ship 1~4 프로덕션 `46df51a`/#12 · `add099a`/#14 · `ef9da44`/#15 · `695e441`/#16. **6b-1** 2026-09-08 · **6b-2** `a00d380` · **6b-3** `7c975c0`/#21 · **6b-4** `70e393b`/#22 · **6b-5**·**6b-6** `0d68d71`/#23 (2026-09-09) — ✅ **6단계 완료, 프로덕션까지**) | **SAAS.md §8 6단계 · §7.7(IA 정본)** · DESIGN §3.1·§6.4·§6.5·§6.6·§6.64·§6.65·§6.66·§6.67·§6.8·§7 · ARCHITECTURE §1.35·§3·§5.5·§5.5.16·§6.1·§6.3·§6.35·§6.4 · CLAUDE.md(`messages/`·`lib/i18n`·`lib/routes`·`lib/shell`·`components/shell`·`components/ui`·`middleware.ts`) · POSTMORTEM 9건(2026-09-08 4 · 2026-09-09 5) | **배송은 다 닫혔다 — 실물 게이트 하나가 남았다.** ⚠️ **Publish 다섯 갈래 중 "일부 미기록"(warning)을 실물로 못 밟았다**(`tasks.md` T7 게이트 절): writer가 값을 버리는 상황이 필요한데 그건 **수술적 어댑터**에서만 나고 검증에 쓴 `order-check`은 재생성이다 — 자리는 `/l10n-roundtrip`(`bugshot-i18n-test`)이다. **첫 착지 < 2초도 미달이다**(3.30초 — 아래 백로그, 수단 셋이 이 기능 밖이라 후속). ⚠️ **2026-09-09 `/doc-check`이 이 칸의 옛 "없음"을 통과시켰다** — 표만 읽고 근거 문서의 게이트 절을 안 봤다. 라우트 여덟 중 `logs` 하나만 남았고 그것은 **7단계**(`SyncRun`의 소비자)다. `user-stories.md`는 완료돼도 남긴다(DESIGN이 참조한다). ⚠️ `tasks.md`도 남긴다 — 체크리스트 밖에 **ship 3 재측정 기록**(LCP 3.30/4.66/3.29초)이 있고 CLAUDE.md·SAAS.md·이 파일이 그것을 참조한다 |
| [sec-audit](./sec-audit/) | 🚧 **착수 전** (2026-09-09 감사 완료, 수정 0건) | 아직 없다 — 결론이 정본으로 올라가는 것은 ship마다다(ARCHITECTURE §1.1·§5.5·§6 · DESIGN §6.65 · POSTMORTEM) | **ship 6개 + DB 트랙 + 판정 13개** 전부. `tasks.md`의 번호가 실행 순서이고 **ship 경계를 정한 것은 시급도가 아니라 게이트다**(재측정 트리거·`pnpm build`·소비자 리포). 🔴 둘(어댑터 프로토타입 키 · 로케일 코드가 리포 경로가 된다) 중 앞의 것만 **실행으로 확정**했다 |
...
| **Publish의 "일부 미기록"(warning) 갈래 실물 검증** | translation-ui `tasks.md` T7 게이트 절 (2026-09-08) | 다섯 갈래 중 넷은 `order-check`로 밟았는데 이 하나는 **수술적 어댑터**(값을 버리는 writer)에서만 나고 그 리포는 재생성이다. `pullMessage`의 갈래는 단위 테스트가, tone→variant 배선과 `<details>` 목록은 소스 스캔이 들고 있어 **남은 것은 실물 한 번**이다 — `/l10n-roundtrip`을 `bugshot-i18n-test`로 돌리면 같은 실행에서 닫힌다 |
