...
여섯째로 **`findings.md`**(우리가 돌린 감사의 발견 목록 — `sec-audit/`·`sec-audit-2/`에 있다)가 있다. 그 디렉터리는
...
| [sec-audit-2](./sec-audit-2/) | 보안 수정·로컬 검증, 배포 전 | ARCHITECTURE §9 · SAAS §11 · [작업 기록](./sec-audit-2/tasks.md) | 마이그레이션·OWNER 재연결·실물 검증, #37 사용자 결정 제외 · #38 후속 session-revocation |
| [credential-storage](./credential-storage/) | 로컬 구현·자동/격리 DB 검증, 운영 전환 전 | [spec](./credential-storage/spec.md) · [design](./credential-storage/design.md) · SAAS 저장 보호 · ARCHITECTURE §5.1 | [operations](./credential-storage/operations.md)의 R1/R2·실제 OAuth·배포 검증 |
| [session-revocation](./session-revocation/) | #38 로컬 구현·자동 검증, 배포 전 | SAAS 전체 세션 회수 · ARCHITECTURE §6.1.1 · [spec](./session-revocation/spec.md) | credential 전환 후 실제 GitHub/Google·두 브라우저·키보드 검증 |
