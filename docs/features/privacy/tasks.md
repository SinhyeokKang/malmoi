...
- [x] **P6.2** (2026-09-19 사용자 — **퍼블릭 게시 완료**) Google Cloud 콘솔 — OAuth 동의 화면에 `https://mal-moi.com/privacy`를 넣고 **게시**로 전환한다.
  - ⚠️ **Search Console의 도메인 소유권 인증은 실패한 채로 남아 있다** — 가비아 TXT에 넣은 토큰이 **36자로 잘렸다**(정상은 `google-site-verification=` 뒤 43자. 인증 창의 입력칸이 폭에 맞춰 잘려 보여 그 만큼만 복사됐고, 실패 화면이 발견된 값을 되읽어 **같아 보이는 것이 함정이다**). 게시는 통과했으므로 지금 막는 것은 없지만, 승인된 도메인을 다시 건드릴 일이 생기면 여기부터 본다 — [복사] 버튼으로 다시 복사해 값을 **교체**한다.
