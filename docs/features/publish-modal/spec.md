...
4. **확인 창의 두 형 + 포커스 착지** — Archive·Sync는 360, Publish는 736이다. 경계 규칙("답만 받는
   ...
   올릴지. **포커스는 리뷰 6번에서 확정**: Publish 최초 착지는 시안의 모달 컨테이너를 따르고,
...
**리뷰 5번 보류**: OWNER/EDITOR별 버튼 분기는 추가하지 않는다. 설정 버튼을 없애고 공통으로
프로젝트 소유자에게 오류 코드를 전달하도록 안내하는 대안은 제안 상태이며, 문구·버튼 변경은
아직 확정하지 않았다. 기존 시안의 설정 버튼은 EDITOR에게 접근 불가라는 제약이 남는다.
