...
- **Home 배선이 없다** (project-home T6). 이 컴포넌트 둘을 렌더하는 페이지가 **아직 하나도 없다** —
  기능 완료로 보고하거나 feature 디렉터리를 지우지 않는다.
- **실물 왕복(T12)·실제 Home에서의 실측(T11)은 미검증.** 아래 §시안 대조의 실측은 **임시 측정용
  라우트**로 잰 것이고 그 라우트는 지웠다.
- 요청 범위는 **push 전까지**다. push·main 머지·prod DB 변경을 하지 않았다.
