...
|---|---|---|
| **T11. UI·시안·접근성 검증** | **절반 끝났다** (2026-09-15 저녁, 아래 §T11 진행) — 여섯 갈래를 실물로 밟았고 결함 하나를 잡아 고쳤다 | 남은 다섯 갈래는 리포·서버 상태를 만들어야 밟힌다. `4c`는 T12가 PR을 만들면 따라온다 |
| ~~**T12. 실물 왕복**~~ | ✅ **끝났다** (2026-09-16 — 아래 §T12) | — |
| **T13. 정본 반영·기능 완료** | T11·T12 선행 | PRODUCT의 "아직 안 만들었다" 표기 제거 외 |

### §T11 진행 — 2026-09-15 저녁 (`i18n-format-check`, 로컬 dev)

**밟은 것** — Chrome computed style + 접근성 속성으로 쟀고 캔버스(`design_handoff_sync_repository`) 값과 대조했다.

| 갈래 | 아트보드 | 결과 |
|---|---|---|
| 미발송 0 ∧ PR 없음 | `4a` | ✅ **본문 자체가 없다** — Dialog 360/radius 12/shadow · 머리 `16 16 8` gap 8 · 제목 15/500/22.5px/0.225px · 설명문 13/20.8px/0.26px/`#737373`/padding `0 16` · 브랜치 mono 13/18px/`#525252` · 푸터 16/gap 8 · 포커스 `Cancel`. **핸드오프가 "jsdom으로만 섰다"고 적어둔 둘 중 하나였다** |
| 미발송 12 | `4b` | ✅ amber 블록 radius 10/padding 12/13px/글리프 14 mt 2 · 블록 사이 8 · `#fffbeb`·amber-200·amber-900 · 수에만 weight 500 · 권유가 앱 내부 링크 |
| PR 조회 중·실패 | `4d` | ✅ 열자마자 미확인 줄이 서고 `aria-describedby`가 설명문+경고 **둘**로 넓어진다. 조회가 `null`을 주면 줄이 사라지고 describedby가 하나로 줄어든다 — **블록이 줄어드는 방향** |
| 진행 중 | `4f` | ⚠️→✅ **결함을 잡았다** — `[Publish]`가 안 잠겼다(아래). 고친 뒤 재측정: 트리거 `Syncing…`/`aria-disabled=true`/`disabled=false`/포커스 유지, `[Publish]` `disabled=true` |
| 연타 | `4f` | ✅ 진행 중 두 번째 클릭이 입력을 못 받는다 |
| 결과 성공 한 줄 | `4e` | ✅ `Synced 18 keys from dev` — `role=status`, `<p>` 하나, 닫기 있음 |
| 결과 유지 | — | ✅ `router.refresh()`가 집계를 갱신(`To send 12 → 0`)해도 결과 Alert가 남는다. 하드 리로드에서만 사라진다(결과는 지속 상태가 아니다) |

**잡은 결함 하나** — 진행 중 `[Publish]`가 그대로 눌렸다. 캔버스 `4f`의 *"[Publish]도 함께 잠근다"*
위반이고, T9 연결 계약이 **호스트의 몫**이라고 적어둔 자리다. `components/home/actions.tsx`가 두 진행을
따로 들고 서로를 잠근다 — 하나의 `busy`로 접으면 Sync가 자기를 잠가 포커스 복귀 대상이 사라진다.
회귀 테스트는 `components/__tests__/home-actions.test.tsx`이고 규칙은 `docs/DESIGN.md` §6.64에 올렸다.

**남은 다섯** — 전부 **리포나 서버 상태를 만들어야** 밟힌다.

| 갈래 | 아트보드 | 밟는 방법 |
|---|---|---|
| 열린 PR (미발송 N · 미발송 0) | `4c` 좌·우 | **T12가 만든다** — Publish를 돌리면 PR이 생기고 그 뒤 Dialog를 열면 둘 다 선다. ⚠️ 우측은 핸드오프가 "jsdom으로만 섰다"고 적어둔 나머지 하나다 |
| 정상 0키 | `4e` | 리포의 로케일 파일을 빈 카탈로그로 만든다 (폐기용 리포에서) |
| 일부 파일 실패 | `4e` | 파일 하나를 일부러 깨뜨린다 — `docs/DESIGN.md` §6.64의 `2b` 밟는 방법과 같은 조작이다 |
| CI 미적용(`superseded`) | `4e` | Sync 도중 `/api/push`가 들어와야 한다 — 경합을 만들어야 한다 |
| 거부 갈래 일곱 | `4f` tone 표 | 서버 상태(실행 중·미연결·표면 0·리포 교체·세션 만료)를 각각 만든다 |

⚠️ **역방향 잠금(Publish 중 Sync)은 jsdom 테스트로만 섰다** — 실물로 밟으려면 Publish를 실제로 돌려야
하고 그것은 대상 리포에 PR을 만든다(= T12). **"브라우저로 확인했다"고 적지 않는다.**
...
### §T12 실물 왕복 — 2026-09-16 (`i18n-format-check`, yaml-catalog + code-dict)

`/l10n-roundtrip` 전 단계 완주. **어댑터를 고쳐서가 아니라 `lib/import/surface.ts` 추출(T2)이 같은 읽기
경로를 지나기 때문에** 이 검증이 필요했다.

| 단계 | 결과 |
|---|---|
| 1 적재 | Home의 `[Sync]`로 (push:local이 아니라 **기능 자체**로) — 18키 |
| 2 값 고정점 | `no-changes` ✅ — ⚠️ 이 게이트의 한계를 이번에 알았다(아래 ②) |
| 3 편집 3건 → Publish | PR #3 · **3파일 `+4 -5`** · 두 표면·세 파일에 걸침. 앵커(`<<: *common`)·주석·빈 줄·키 순서·따옴표 스타일(`'言語'` / `"設定"`)·TS 주석 전부 보존 ✅. `locales/en.yml`과 `src/i18n/{en,ja}.ts`는 **안 건드려짐**(파일 단위 변경 감지 ✅). 교차 격리 ✅ — 같은 이름의 `common.ok`가 `locales`에서는 그대로 |
| 4 머지 → 재Sync | `[skip-malmoi-i18n]` ✅ · sync 브랜치 삭제 ✅ · 재Sync 뒤 **세 값이 바이트 그대로**(따옴표·백슬래시·em dash 포함) 돌아오고 `updatedBy`가 `null`로 비워짐 ✅ |
| 4 재pull 수렴 | 1차 `no-changes`(2층) ✅ · 즉시 2차 `no-edits`(1층, GitHub 미호출) ✅ |
| 5 CI | 건너뜀 — 어댑터 검증이 목적이고 CI 층은 어댑터와 무관하다 |
| 6 정리 | 토큰 발급·`.env.local` 편집 **없음**(`[Sync]`를 썼다) · 일회성 Project 없음 · 편집은 머지됐으므로 그대로 둔다 |

**찾은 것 둘 — 둘 다 sync-repository 밖이라 고치지 않고 기록했다**

1. **`yaml-catalog`이 편집하지 않은 키의 손으로 감은 접힌 스칼라를 편다.** `locales/ja.yml`에서
   `errors.unknown` 하나만 고쳤는데 `settings.help: >`의 두 줄이 한 줄이 되어 diff에 함께 실렸다.
   원인은 `doc.toString({ lineWidth: 0 })` — 접기를 끄면 **원본의 손 줄바꿈도 함께 풀린다.**
   의미는 같고 **결정성 불변식도 안 깨진다**(같은 DB → 같은 바이트); 깨지는 것은 "값만 바꾼다"다.
   **처음이 아니다** — 같은 리포 커밋 #1·#2에서 `ko.yml`이 이미 그렇게 펴졌다. → `docs/ARCHITECTURE.md` §1.4
2. **`/l10n-roundtrip` 2단계는 바이트 고정점이 아니라 값 고정점이었다.** 수술적 어댑터는 바꿀 값이
   없으면 `changed = false`로 원본 바이트를 그대로 돌려줘 **재직렬화를 한 줄도 안 지난다** — 그래서
   ①이 그 게이트를 통과한 채 살아 있었다. 스킬 문서를 고쳤다(`c80d4af`).

### §T11 추가 — 2026-09-16 (T12가 연 갈래 셋)

| 갈래 | 결과 |
|---|---|
| `4c` 오른쪽 (미발송 0 ∧ 열린 PR) | ✅ 미발송 줄이 **빠지고** PR 줄만 선다(`0 edits …`를 안 쓴다) · 글리프 1 · 권유가 `Nothing is waiting to be sent.` + `See what's open`(`_blank`·`noreferrer`·글리프 12·파랑). **핸드오프의 "jsdom으로만 섰다" 둘이 이제 둘 다 실물로 섰다** |
| 역방향 잠금 (Publish 중 `[Sync]`) | ✅ `disabled=true` · 글자 `#737373` · 클릭 불가 · Dialog 안 섬 |
| 거부 — 요청이 못 감 | ✅ CDP로 오프라인을 만들어 확정 → `role="alert"` · 화면 전용 문구 · 닫기 있음 · danger 규격(border `destructive/40` · bg `#fff` · radius 12 · padding 16) |

**남은 넷**: 정상 0키 · 일부 파일 실패 · CI 미적용(`superseded`) · 거부 나머지 다섯.
⚠️ **`4c` 왼쪽(미발송 N ∧ 열린 PR)은 지금 못 밟는다** — PR을 머지해 열린 PR이 없다. 다음 왕복 때
**머지 전에** 미발송을 만들어 Dialog를 열면 같이 밟힌다.
