...
- [ ] L2.9 (⚠️ sync-edit-protection 배포 B에서 **안 닫혔다** — PRODUCT §7.6에 "야간 cron은 미전달 편집이 있는 프로젝트만 GitHub에 닿고 열린 PR도 새 편집이 있으면 갱신한다"만 올렸다. 예산·`maxDuration`·온보딩 공지는 그대로다) 야간 cron의 예산과 공지. `app/api/pull/route.ts:26,70`이 최대 50개를 직렬로 돌고 `vercel.json`에 `maxDuration`이 없다 — 60초 킬이면 `:99,102`의 요약 로그·응답이 **통째로** 안 남는다. 값은 공정성이 아니라 **요약이 남는 것**이다(§5.6.5 정렬이 다음 밤 이월을 이미 보장). 온보딩 어디에도 야간 자동 PR 언급이 없다 — PRODUCT §4.1에 그 약속이 "어긋난" 게 아니라 **없다**(`en.tsx` `nightly`는 Logs 라벨 `:497`·실패 사유 `:521`·보관 본문 `:542`뿐). (audit #11)
...
- [x] L3.2 (2026-09-18 — 재현됨. 자리는 `checkRepoAccess`의 설치별 `listInstallationRepos` `.then(_, () => ({repos: []}))`이고, **로그도 안 남기고** 전부 실패를 `repo-not-installed`로 접었다. 가르는 축은 "성공한 조회가 하나라도 있었나"다 — 전부 실패면 `unavailable`, 부분 실패면 `logFailure`만 남기고 읽어낸 목록으로 진행(일시중지 설치의 403은 영구 상태라 통째로 접으면 정상 설치도 못 쓴다). 형제 `listConnectableRepos`와 같은 형이라 `planRepoConnect`에 `unavailable` 입력을 늘릴 필요가 없었다. ⚠️ **대조군을 짝으로 박았다** — 부분 실패가 `ok`인 테스트가 없으면 "전부 unavailable"로 퇴행해도 green이다) 설치 조회 실패를 "설치 안 됨"으로 접지 않는다. `app/(edit)/projects/actions.ts:1404-1411` `checkRepoAccess`가 설치 하나의 리포 목록 조회 실패(403·5xx)를 로그 없이 `{ repos: [] }` → `repo-not-installed`로 만든다. 형제 `listConnectableRepos`(`:550-573`)는 `logFailure`를 남긴다. **`fa55840`(home state)는 이 자리를 고치지 않았다.** 선례는 POSTMORTEM 2026-09-06(`:510` "401이 not-installed로 접혀 있었다"), 09-03은 셸 항목이라 무관. `planRepoConnect`(`lib/onboarding/connect-plan.ts:34-53`)가 이미 순수 판정이므로 `unavailable` 입력 하나 늘린다. (audit #15)
  - 검증(자동): 5xx → `unavailable` + 로그 1줄, 404 → `repo-not-installed`, **정상 목록에 리포 있음 → ok** 대조.
- [ ] L3.3 (2026-09-18 절반 — **코드·자동 검증 완료, 수동 둘 남음**. `loadPublishPreview`가 `null` 하나 대신 `ok`·`rejected`·`failed` union을 낸다. `rejected`는 `triggerPullAction`의 실행 전 거부와 같은 낱말(`unauthorized`·`forbidden`·`not-found`·`archived`)이라 클라이언트가 그대로 `result`/`config-error`(`1h`)로 흘린다. 세션 저장소 장애(`unavailable`)는 거부가 아니라 `failed`(`1k` Retry)다. `/signin`·설정 링크는 `routes.*`로 바꿨다. **남은 것**: `/design-sync`(미리보기 단계 진입 시 포커스·`DESIGN.md:1035` 갱신) · `/bugshot-qa`(세션 쿠키 삭제 → Publish → Sign in)) (⚠️ sync-edit-protection 배포 B에서 **안 닫혔다** — 그 spec은 Publish 모달의 `partial` 틀을 "보내지 않았다"로 바꿨을 뿐 미리보기 실패 갈래는 건드리지 않았다) Publish 미리보기가 세션 만료·접근 거부를 원인 없이 "미리보기 실패 / Retry"로만 그린다. `app/(edit)/publish-actions.ts:9-18` `loadPublishPreview`가 모든 실패를 `null` 하나로 접는다(→ `publish-button.tsx:54`). **새 갈래를 그리지 않는다(결정됨)** — 핸드오프 `1h`(실행 전 거부 여섯, `DESIGN.md:1017-1020`)가 `Sign in`(역할 무관)·`Open project settings`(OWNER)까지 확정돼 있고, run 실패에는 이미 `unauthorized`→`/signin` 분기가 있다(`publish-button.tsx:458-462`). 미리보기 단계의 `unauthorized`·`forbidden`을 그 `config-error` 갈래로 흘리고, 읽기 실패 `1k`("실패로 말하지 않는다", `en.tsx:1603`)는 그대로. **`:459,462`의 `/signin` 하드코딩 → `routes.*` 전환을 이 태스크에 포함**한다(L7.1에서 옮김 — 같은 줄을 둘이 건드리면 충돌). POSTMORTEM 2026-09-16 `:1929`(같은 파일 로케일 무시)와 같은 영역. (audit #16)
...
- [x] L3.6 (2026-09-18 — ① 재현됨: 루트만 `normalizeArrays`를 건너뛰고 자식부터 돌린다(루트 아래 숫자 키는 여전히 배열). ② **닫음, 도달 불가** — multi-locale 어댑터는 `ts-dict` 하나이고 수술적이라 "원본 부재"와 "`writeStrategy === surgical`"이 같은 판정이다. 재생성 multi-locale이 생기는 순간이 이 분기를 고칠 시점이다. ③ 재현됨: 계약 매트릭스에 "원본 파일 둘" 축(다른 로케일 파일을 앞에 싣는다) + `Break.firstFile`을 넣었더니 yaml-catalog·code-dict가 red였다 — 둘 다 `pathTemplate`에 로케일을 넣은 경로로 고른다. ⚠️ **규칙을 다 지키는 가짜 어댑터도 `[0]`을 읽고 있었다** — 그것도 경로 조회로 고쳤다) 어댑터 잠재 결함. (audit #20)
...
- [x] L3.7 동작 어긋남. (audit #21)
  - ✅ (2026-09-18 — 조회를 head만으로 하고 PR의 base를 받아, 어긋나면 `PATCH /pulls`로 base를 옮긴다. `loadOpenPrUrl`도 같은 조회라 base 인자가 빠졌다) `lib/pull/run.ts:210-211` — base 브랜치를 바꾸면(`settings/actions.ts:281`) 저장된 `baseBranch`로 옛 PR을 못 찾아(`lib/github.ts:393-402`가 base로도 필터) PR이 하나 더 열린다.
  - ✅ (2026-09-18 — 사용자 판정: **셀을 빼고 따로 센다**. 막으면 파일 하나 빠진 프로젝트의 Publish가 통째로 멈춘다. `PublishPreview.withoutFile`이 뺀 수이고 `truncated`는 조회하지 않은 행만 센다. 표 아래 한 줄이 그 수를 말한다 — 새 문구라 `/design-sync` 대상) `lib/publish/read.ts:45-46` — per-locale 수술적 어댑터에서 로케일 파일이 없으면 미리보기는 변경을 약속하는데 pull은 `missingOriginal`로 안 낸다(`render.ts:104-108`).
  - ✅ (2026-09-18 — 재현됨, **태스크의 수정 방향은 버렸다**: `markImportStarted`를 트랜잭션 안으로 옮기면 롤백된 실패의 `import-failed`가 사라진다(ARCHITECTURE §5.5.7에 이미 적힌 거부). 대신 둘을 따로 막았다 — 성공 결과는 토큰과 무관하게 쓰고 진행 표시만 조건부, 트랜잭션 안 `stale-commit`은 `abandonImportRun`. 각각을 되돌리면 PG 테스트가 red다) `app/api/push/route.ts:170,182` — `markImportStarted`가 트랜잭션 밖에서 `lastImportToken`을 덮는다. **결과는 서술보다 나쁘다**: A 성공 → B가 토큰 덮음 → A의 종료 쓰기(`apply.ts:338` 토큰 대조) 0행 → **성공한 임포트가 `import-failed`로 표시**되고, 역방향은 실패가 무음으로 유실된다. 수정은 `markImportStarted`를 `applyPush` 트랜잭션의 `FOR UPDATE` 뒤로 옮기는 것 하나 — B는 잠금 대기 뒤 `stale-commit`을 받아 자기 토큰으로 닫는다.
...
- [x] L3.8 (2026-09-18 — **첫 방향이 통했다**: 사본을 `new NextRequest(request.url, init)`로 조립한다. url 문자열이라 런타임 객체의 사설 필드를 안 읽어 09-12 함정을 비껴가고, `nextUrl`이 생겨 `reqWithEnvURL`이 선다. 재현은 **진짜 `NextAuth` 핸들러**로 했다 — next-auth가 `next/server`를 확장자 없이 import해 vitest가 못 풀어서 `vitest.config.ts`에 `server.deps.inline: ["next-auth"]`를 넣었다. ⚠️ **첫 red가 반쪽이었다**: env를 `""`로 비우면 `AUTH_URL ?? NEXTAUTH_URL`이 `""`에서 멈춰 NEXTAUTH_URL 케이스가 거짓 green이었다 — `undefined`로 지운다. **런타임 확인은 안 했다**: 09-12 함정은 Next 런타임에서만 드러나는 부류인데, 이 변경은 런타임 객체를 생성자에 넣지 않으므로 그 경로를 다시 열지 않는다) `lib/login-link/http.ts:94` — `new Request(...) as unknown as NextRequest`. `AUTH_URL`·`NEXTAUTH_URL`을 설정하는 순간 next-auth가 `req.nextUrl`을 구조 분해해 계정 연결 콜백이 전부 TypeError가 되고 `:120`이 무로그 500으로 삼킨다. 유일한 방어선이 `.env.example:58` "의도적으로 없다" 한 줄이다. **같은 파일·같은 부류의 선례가 POSTMORTEM 2026-09-12 `:1173`("테스트가 만든 NextRequest는 복사되고 런타임이 준 것은 던졌다")이다.** 검증만 두지 말고 고친다. (audit #22)
...
- [x] L3.9 (2026-09-18 — **자리가 둘이 아니라 다섯이었다**: `lib/sync/run.ts:175` · `app/api/pull/route.ts:113,136` · `app/api/push/failure/route.ts:142` · `app/api/push/route.ts:244`. ⚠️ **정책 충돌이라 사용자 판단을 받았다** — `lib/failure.ts`(2026-09-04)는 "전문은 서버 로그로"였고 `log.ts`(2026-09-10 credential 리뷰)는 "로그에도 원문 금지"였다. 넉 달을 반대인 채로 굴렀고 보수적인 쪽으로 합쳤다. 공유 지점인 `classifyFailure`의 `detail`을 분류(`http-<status>` / 생성자 이름 / `typeof`)로 바꿔 다섯이 한 번에 닫혔다 — 호출부는 안 고쳤다. **대가**: Prisma 접속 실패의 호스트·유저가 이제 어디에도 안 남는다. 옛 계약을 고정하던 테스트 셋(`failure.test.ts` 둘 · `route-diagnostics.test.ts` 둘)을 반전 근거와 함께 뒤집었다) `lib/sync/run.ts:168-169`와 **`app/api/pull/route.ts:113`**이 `failure.detail`(에러 원문)을 `console.error`한다. `lib/github-connect/log.ts:16-24`의 "원문 금지" 규칙과 반대다. (audit #23)
  - 검증(자동): 두 자리 모두 로그 인자에 `error.message` 원문이 없음(스파이), **갈래 이름은 있음** 대조.
- [x] L3.10 (2026-09-18 sync-edit-protection 배포 B로 닫힘 — 술어의 주인이 `lib/protection/where.ts` `pendingWhere` 하나이고 `countUnpublished`·pull 1층·`lib/publish/read.ts`·Publish 캡처가 그 객체를 쓴다. 손 사본은 셀 `pending` 투영·목록 raw SQL·하네스 셋이고 `lib/keys/__tests__/sync-edit-protection.integration.ts`가 ①②③·1층·캡처를, `where.test.ts`가 ④의 소스를 대조한다) 미발송 술어는 **프로덕션 넷**(`lib/keys/view.ts:414` `isUnpublished` · `query.ts:180` `countUnpublished` · `query.ts:573` 목록 raw SQL · `lib/publish/read.ts:19`) **+ 하네스 하나**(`app/(edit)/__tests__/harness.ts:914,1108`)다. `test:projects:postgres`(`list-aggregates.integration.ts:313,466`)는 ①②③만 대조한다 — **④는 PR에 실제로 실리는 행을 정하는 사본**이라 가장 먼저 대조에 넣어야 한다. `readPublishPreview`가 GitHub 호출(`read.ts:26-28`)을 섞고 있어 `where` 빌더를 먼저 순수 함수로 분리해야 대조 가능. 갱신 대상: `CLAUDE.md:140` "세 벌" · `lib/keys/query.ts:563-564` 주석 · `app/(edit)/__tests__/queries.test.ts:123`. (audit #24)
...
- [x] L4.2 (2026-09-18 — 셋 다 리포에 남는 형으로 넣었다. ① "surfaceId로도 좁혀진다": 표면 스코프 다섯 문장(로케일 orphan·키 UPDATE·키 orphan·needsReview 전파·refs DELETE)을 이름으로 짚고 전 문장을 훑는다 — 각 `AND "surfaceId"`를 하나씩 지우면 전부 red. **needsReview 전파는 원문이 바뀐 키가 픽스처에 없어 문장 자체가 안 나왔다** — stale 키를 더했다. ② 스텁에 `lastCommitAt`을 받게 해 역행 가드가 돈다(쓰기 0 · 같은 시각 통과 · `preserve` 우회 셋). 가드 줄을 지우면 red. ③ route mock에 진짜 `ApplyGuardError`. **옛 500 테스트는 실제로 우연이었다** — catch의 `instanceof`가 vitest의 "export 없음" `Error`를 던져 실패 기록에 못 닿고도 500이었다. 로그 클래스 이름으로는 못 가르므로 "실패 기록이 쓰였다"를 단언한다. 트랜잭션 안 `stale-commit` → 409 · 진행 표시만 거둠 케이스 추가) `lib/push/__tests__/flow.test.ts:331-338`이 `projectId`만 본다 — `apply.ts:239,246`의 `AND "surfaceId"`를 지워도 green, `:74` 스텁 표면이 `lastCommitAt: null` 고정이라 트랜잭션 안 가드(`apply.ts:106-114`)가 한 번도 안 돈다(`:114` 역행 가드는 `refsMode === "replace"`에서만 — Sync `preserve`는 우회). `app/api/__tests__/route-diagnostics.test.ts:45`의 mock에 `ApplyGuardError`가 없어 500 테스트가 우연히 통과할 수 있다. (audit #26)
...
- [x] L4.3 (2026-09-18 — 다섯 다 고쳤다. **각 새 단언이 옛 상태에서 red인지**: survey는 옛 픽스처(안 던짐)에서 `spy` 호출·`failure` 단언이 red. members는 호출을 첫 JSX `return` 뒤로 옮기면 red. repository-import는 `apply-plan.ts`의 `lease-lost`를 `superseded`로 바꾸면 2개 red. yaml·code-dict "중간 경로"는 **동작을 고정한 것이지 옛 구현이 틀린 게 아니다** — 제목만 반대였고 실제 출력(가장 깊은 기존 맵에 `brand.new.deep` 리터럴)을 탐침으로 확인한 뒤 그 모양을 단언했다. nested-keys는 바이트 고정점만 보던 자리에 "2차 read = 1차 − 얕은 쪽"을 더했다 — 이 항목은 red를 만들어 보지 않았다) 이름과 본문이 다른 테스트를 고친다 — 항목별 대체 단언: (audit #27, POSTMORTEM 2026-09-03 재발)
...
- [x] L4.4 (2026-09-18 — 공허한 다섯에 존재 단언을 세웠다: `render.test.ts` 둘(파일을 먼저 찾고 내용이 있음을 센다) · `github-callback.test.ts` 둘(`toHaveBeenCalledTimes(1)`) · `token-store.test.ts`(루프 전 `toHaveBeenCalled`). **`detect.test.ts` 둘(지금 `:234,406`)은 재현 안 됨** — 직전 줄의 `toBe`·`toEqual`이 존재를 보장한다. ⚠️ 같은 grep이 범위 밖 여덟을 더 낸다: `edit-flow.test.ts:332,340` · `sync-button.test.tsx:100,138,193` · `new-project.test.tsx:630` · `sync-edit-protection.integration.ts:296,315` — 손대지 않았다) `?.` 뒤 부정 단언 — **재현 먼저**: 8곳 중 `render.test.ts:214,223`·`run.test.ts:605`·`new-project.test.tsx:573`(문서 `:573`, 감사 `:565`)은 직전 줄이 존재를 보장해 이미 red다 → "재현 안 됨"으로 닫는다. 공허한 것은 `render.test.ts:105,270`·`github-callback.test.ts:307,337` 넷. POSTMORTEM 2026-09-14 후속 목록(`:1359-1374`)에 **`token-store.test.ts:144`·`detect.test.ts:189,361`**도 있다 — 추가. (audit #28)
  - 검증(자동): 각 자리에 존재 단언 선행. 존재 단언을 빼면 red(리포에 남는 형).
- [x] L4.5 (2026-09-18 — **JSON CRLF는 픽스처 편향이 아니라 실제 결함이었다**: json-catalog·chrome-locales가 CRLF 원본을 LF로 다시 써서 값 무변경에도 전 줄 diff였다. `JsonStyle.eol`(우세한 쪽)을 더해 고쳤고 LF 원본은 LF 유지가 짝이다. ts-dict는 CRLF 원본 값 무변경·값 변경 둘 다 이미 바이트 보존이라 고정만 했다(작은따옴표 변형은 전부터 있었다). scan은 작은따옴표 import·호출과 템플릿 리터럴 거부를 고정했다 — `literalOf`가 템플릿을 받게 바꾸면 red) 픽스처 편향: JSON 어댑터 CRLF(`JsonStyle`에 개행 필드 없음, `lib/adapters/json-style.ts:24-44`) · ts-dict 스타일 한 가지 · scan 픽스처 전부 큰따옴표 · `` t(`key`) `` 거부(`lib/scan/ast.ts:250-256`) 미고정. 범위는 JSON — yaml·code-dict CRLF는 이미 있다(`yaml-catalog.test.ts:556`, `code-dict.test.ts:569`). (audit #29)
  - 검증(자동): 각 축에 둘째 스타일 픽스처 — CRLF 원본 값 무변경 write → 바이트 동일(**LF 원본은 LF 유지** 짝).
- [x] L4.6 (2026-09-18 — survey의 write 입력을 프로덕션과 같게 만들었다: read 결과에서 push가 남길 키(base 파일의 키만 · base 순서 `sortIndex` · 빈 값은 셀 없음)를 만들고 로케일마다 `rowsForLocale` → `buildWriteEntries`를 지난다. multi-locale도 `writeWithErrors`로 에러를 센다. 옛 `toBe(0)`은 `BASE_SCRAMBLED` 픽스처의 단언이었고(감사 줄 번호와 다르다) `toBeGreaterThan(0)`으로 반전했다. 둘째 배선은 되돌리면 red. **§1.9 지표 20차 재측정은 안 했다** — 표 아래에 "그 전 값"이라고 적었다) survey 측정을 프로덕션 write 경로에 맞춘다. `lib/survey/one.ts:499` `writePerLocale`이 `buildWriteEntries`를 안 거쳐 `diffRatioNonBase`가 비-base 재배열을 ≈0으로 보고하고, `order-metrics.test.ts:196`이 그 0을 고정한다. `:534` `writeMultiLocale`이 `writeWithErrors`가 아니라 `write`를 불러 ts-dict `writeErrors`가 구조적으로 0이다. (audit #30, POSTMORTEM 2026-09-02 재발)
...
- [ ] L7.2 (⚠️ 2026-09-18 sync-edit-protection 배포 B에서 **안 닫혔다** — `countUnpublished`는 이제 `countPending`을 지나 토큰이 없으면 인덱스 count 하나로 끝나지만 표면마다 부르는 구조는 그대로다) N+1: `app/(edit)/projects/[slug]/surfaces/[surfaceSlug]/locales/page.tsx:57-58` · `translations/page.tsx:185-186`가 표면마다 `countUnpublished` 1쿼리(`Promise.all`이라 직렬은 아님). `translations/page.tsx:158-161`과 `:185`가 **두 라운드**로 갈려 `:155-156` 자기 주석과 모순. 표면별 집계 한 번(`countUnpublishedBySurface`)으로. (audit #40)
