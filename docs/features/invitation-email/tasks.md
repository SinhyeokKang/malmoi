# Invitation email — tasks

상태: 미구현 · 2026-09-23 검수 피드백 반영. [spec.md](./spec.md)·[design.md](./design.md)가 계약이다. 이번 검수에서는 기능 문서만 수정·커밋하고 코드·정본 문서는 변경하지 않는다.

## T0. 디자인 범위

- [x] **T0.1** Resend·본문 링크 한 줄·다중 초대·사람별 역할·성공 시 닫기를 확정했다. 같은 주소 60초, 프로젝트 최근 1시간 20건, 재발급 시 이전 토큰 만료도 확정했다.
  - 검증: 대화의 사용자 결정과 spec/design을 대조했다.
- [x] **T0.2** 불필요한 분기를 줄이는 사용자 위임과 **Pending Resend 유지**를 반영했다. 부분 결과 화면·주소별 발송 상태·링크 복사를 제거하고 전체 사전 검증으로 통일했다.
  - 검증: 데스크탑 디자인 피드백도 같은 방향으로 대체했다. 디자인 링크는 spec §1.1에 보존한다.
- [ ] **T0.3** 단순화한 디자인 수정본을 확인한다. `design_handoff_invite_modal 2/`의 README·전달 프롬프트 대조 완료: 단일 폼과 Resend 유지가 일치하고 카드 Alert 위치를 design §6에 반영했다. 발급 건수·재시도 시각·workspace 문구는 같은 절에서 보정한다. **수정본 렌더·실제 조작은 미검증**이다.
  - 검증: 폼 입력→전송→닫힘, 오류 시 같은 폼, Pending Resend를 검토하고 1280×720 긴 목록에서 footer/포커스를 확인한다.

## T1. 순수 함수 — 테스트 먼저

- [x] **T1.1** 다중 입력·역할 보존·중복 거부·전체 발급 제한·메일 payload·설정·요청 단위 결과 판정 테스트를 먼저 작성한다.
  - 검증: 구현 전 red. 특히 한 명이 이미 멤버/간격 제한이면 전체 차단, 19건 상태의 3명 요청 거부, 시간창 정각을 단언한다.
- [x] **T1.2** design §2의 순수 함수를 구현한다. 주소별 발송 상태/성공자 집계 함수는 만들지 않는다.
  - 검증: 순수 테스트 green. 같은 주소의 역할 충돌을 조용히 합치지 않고 메일 본문이 URL 한 줄이다.

커밋 경계 1: `feat: define invitation email policies` — 2026-09-23 완료(`76bb5f0`·`3b7374b`·`3dc1275`). recipients는 클라이언트 그래프 안전(zod·node:crypto 없음 — 한도는 `limits.ts`).

## T2. 서버 발급·메일·Resend

- [x] **T2.1** 서버 전용 Resend `/emails/batch` 호출과 env 배선을 구현한다. 10초 timeout·자동 재시도 없음·요청 UUID 멱등 키. `.env.example`도 갱신한다.
  - 검증: mock HTTP로 접수/명시적 거부/불완전·timeout·5xx를 검사하고 주소·토큰·API 키가 로그에 없는지 확인한다.
- [x] **T2.2** `createInvitations`를 전체 사전 검증→Project 잠금 안 전체 회전·생성·사건→commit 후 발송으로 구현한다. 설정 불가도 생성 전에 막는다. 기존 단건 호출부와 하네스를 전환한다.
  - 검증: 한 대상이라도 사전 거부면 쓰기·발송0, 중간 DB 실패는 전체 롤백, 메일 실패는 저장 유지, 성공/실패 응답 모두 토큰·URL 없음. `membership.test.ts`와 Action 회귀 green.
- [x] **T2.3** `resendInvitation({slug, invitationId})`를 추가한다. 서버의 기존 주소/역할로 재발급하고 같은 제한·발송 경로를 쓴다. design §3의 대상 초대 조건부 회전 count=1을 새 초대·사건 생성의 선행조건으로 둔다.
  - 검증: OWNER만 가능, 타 프로젝트/철회/수락/만료된 id/복호화 실패는 무변경. 재조회 뒤 수락되어 회전이 0건이면 새 초대·사건·발송도 0건. 새 만료7일·inviter·사건, 기존 토큰 무효, API 결과 마스킹을 단언한다.
- [x] **T2.4** `lib/invitation-email/__tests__/invitation.integration.ts`에 격리 PostgreSQL의 동시 초대·Resend·Revoke·수락·한도 경합 검사를 추가하고 `vitest.projects.config.ts`의 include에 등록한다.
  - 검증: 마지막 한도 자리의 동시 요청은 하나만 통과하고 60초 우회가 없다. 수락 선행/Resend 선행 순서를 각각 강제하여 전자는 재발급·초대 사건·발송0, 후자는 옛 링크 수락 실패를 단언한다. 교착 등 DB 오류로 롤백된 요청은 새 초대·사건이 남지 않고 발송0이다. `pnpm test:projects:postgres` green과 실행 보고서의 **새 파일·경합 테스트 실제 수집 및 실행**을 확인한다. 환경이 없으면 미검증으로 남긴다.

커밋 경계 2: `feat: send and resend invitation emails` — 2026-09-23 완료(`3c508c3`·`5a049be`). ⚠️ T2.2의 "기존 단건 호출부 전환"은 T3로 미뤘다 — 모달이 아직 링크 화면을 쓰므로 T2만 배포돼도 preview 초대가 살아 있게 단건 `createInvitation`을 남겼다. 교착 경합은 희생자와 무관한 불변식(반쪽 상태 없음)으로 단언한다.

## T3. UI

- [x] **T3.1** 모달을 행별 이메일·역할·추가/삭제·붙여 넣기 폼으로 바꾼다. 성공은 닫기+토스트, 오류는 입력 유지+행 오류/폼 Alert. 기존 링크 화면과 복사 로직을 제거한다.
  - 검증: DOM에서 역할 보존·중복·붙여 넣기·Enter/IME·pending 잠금·통신 오류 해제·성공 닫기·오류 입력 보존·행 삭제 후 포커스/가짜 성공 없음. 기존 `installFocusFixup`을 유지하고 pending 해제 뒤 첫 오류 행으로 복귀함을 단언한다. 복귀 호출 제거 시 red와 실제 브라우저 착지점도 확인한다. 단순화와 무관한 기존 코드는 고치지 않는다.
- [x] **T3.2** Pending에 Resend를 붙인다. 처리 중 해당 행 Resend/Revoke 잠금, 결과는 목록 재검증 후에도 살아 있는 부모 피드백으로 전달한다.
  - 검증: 성공/거부/unknown, 교체된 행 id에서도 메시지 미소실, 마스킹 라벨, 새 모달·링크·배지 없음. 사전 거부로 행이 남으면 잠금 해제 뒤 Resend 버튼, 성공 또는 발급 후 메일 실패로 행이 교체되면 Pending 카드 제목에 포커스가 닿는다. DOM은 focus-fixup 보정과 복귀 호출 제거 시 red를 확인한다. 실제 브라우저에서도 각 착지점·긴 목록·1280×720·키보드·Resend/Revoke 높이36을 확인한다.
- [x] **T3.3** 기존 로그인·초대 수락 회귀 및 전체 게이트를 실행한다.
  - 검증: GET/HEAD 미소비, 다른 이메일·만료·옛 토큰 거부. `pnpm typecheck` + `pnpm test` green.

커밋 경계 3: `feat: streamline the invitation modal and add resend` — 2026-09-24 완료(`472da45`..`a793ca0`). `/design-sync`로 Chrome 실측·리뷰 2회. 단건 `createInvitation`과 링크 얼굴을 지웠다. T4.1의 PRODUCT·ARCHITECTURE·DESIGN·DIRECTORY 반영도 이 경계에서 했다(코드가 링크 경로를 지워 정본이 거짓이 됐기 때문).

## T4. 문서·운영·실제 왕복

- [x] **T4.1** PRODUCT §4.1/4.2/4.3, ARCHITECTURE §6.02, DESIGN, DIRECTORY를 최종 구현과 맞춘다. 직접 링크 전달은 새 UI에서 제거됐음을 명시한다.
  - 검증: 정본에 미래 구현을 완료형으로 쓰지 않고 메일 장애 시 초대 지연과 Resend 복구 경로를 설명한다. 좌석 제한은 발급 시점의 기존 멤버 수 판정이며 대기 초대의 수락 후 10명을 넘을 수 있다는 기존 정책도 명시한다.
- [x] **T4.2** 실제 개인정보 방침과 OPERATIONS에 Resend 전송처·주소/링크·실제 보존 설정·키 교체·오류 확인 절차를 반영한다.
  - 검증: 실제 외부 전송과 문서 대조, 관련 기존 개인정보 테스트 green.
- [ ] **T4.3** 제목·인증 가능한 발신 주소를 운영에서 확정하고 환경별 키/from/origin·DNS(SPF/DKIM/DMARC)·추적 비활성화를 설정한다.
  - 검증: 키 원문을 기록하지 않고 설정 유효성을 확인한다. `.env.local`은 에이전트가 편집하지 않는다.
- [x] **T4.4** 사용자 지정 수신자로 dev에서 역할 혼합 다중 초대→개별 수신→로그인→수락, Pending Resend→옛 링크 거부를 확인한다.
  - 검증: 메일은 자기 링크 한 줄, 다른 주소 없음, dev 착지, 서버 저장 역할 일치. 실제 메일은 지정 수신자가 있을 때만 보낸다.
- [ ] **T4.5** 프로덕션 배포 뒤 지정 수신자로 prod 왕복을 확인한다. 장애 시 초대 발송을 중단하고 기존 로그인·멤버 접근은 유지한다.
  - 검증: prod 초대가 prod에서 수락된다. 미실행이면 운영 활성화 완료로 기록하지 않는다.

T4.2 — 2026-09-24: 방침(`messages/en.tsx` privacy, 시행일 2026-09-24)과 OPERATIONS "초대 메일 — Resend". ⚠️ **보존 30일은 Resend Free 플랜 값이다** — 계정의 실제 요금제를 사용자가 확인해야 한다.

T4.4 — 2026-09-24 dev(`dev.mal-moi.com`, preview 키) 실측: ① `bugshot-i18n-test-qa`에 ox501tube(이미 EDITOR)+hancycle 한 요청 → ox501tube 행 `already-member` · 바닥 `Nothing was sent by this request…` · Pending 수 불변(쓰기 0). ② `i18n-format-check`에 ox501tube(Editor)+hancycle(Owner) → 토스트 `Invitations sent to 2 people` · 각 메일의 수신자는 자기 하나 · 링크는 dev 초대 URL(버튼·대체 링크 같은 값). ③ 다른 계정(OWNER)으로 열면 `The one you're using wasn't.`(GET은 소비하지 않는다) → ox501tube Google 로그인 → Accept → 프로젝트 Home 착지 · 멤버 목록 Editor(서버 저장 역할 일치) · 로그에 invited×2·joined. ④ 같은 링크 재열람 → `This link was already used`. ⑤ OWNER로 hancycle Resend → 토스트 · 역할 Owner 유지 · 스레드에 메일 2통(토큰 다름) · 옛 링크 → `This invitation expired`. ⚠️ **ox501tube 쪽 메일이 Gmail 스팸함으로 갔다**(hancycle은 받은편지함) — 원본 헤더는 SPF pass(23.251.234.56 → `rsend.notify.mal-moi.com`) · DKIM pass(`notify.mal-moi.com` s=resend + amazonses.com) · DMARC pass(p=none, header.from mal-moi.com)였다 — 인증 결함이 아니라 **새 발신 도메인의 평판**으로 판단한다(같은 발송이 hancycle에는 받은편지함). 설정 변경 없음.

커밋 경계 4: 문서별로 분리. Codex 커밋에는 지정 트레일러를 붙인다. 원격 push/merge는 Claude Code의 `/push`·`/merge`가 담당한다. 이 스펙 단계에서는 배포·계정 생성·DNS 변경·실제 메일 발송을 하지 않는다.
