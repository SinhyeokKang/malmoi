...
**Resend:** 인가된 projectId와 invitationId로 행을 조회하고 저장된 주소·역할을 서버에서 읽는다. Project 잠금 뒤에도 대상이 미수락·미만료인지 다시 확인한다. 철회/수락/다른 재발급으로 닫힌 id면 쓰기 없이 거부한다. 같은 제한 함수를 통과한 뒤 **대상 id·인가된 projectId·acceptedAt:null·조회한 expiresAt과 동등·쓰기 시점에 미만료**를 조건으로 옛 링크를 만료시킨다. **조건부 갱신 count=1일 때만** 같은 트랜잭션에서 새 초대·사건을 생성한다. 0건이면 새 초대·사건·발송 없이 거부한다. 만료는 새 발급부터 7일, invitedBy는 현재 OWNER다. 주소 복호화 실패도 쓰기 전에 거부한다. 서버의 주소 원문을 Pending props에 노출하지 않는다.

기존 수락은 Project 잠금에 참여하지 않으므로 재조회만으로 경합이 닫히지 않는다. 동시 Resend·Revoke·수락 경합은 대상 초대의 조건부 갱신과 기존 수락 CAS를 실제 PostgreSQL에서 검증한다. 수락이 먼저 확정되면 재발급·메일·초대 사건은 0건, Resend가 먼저 확정되면 옛 링크 수락은 실패해야 한다. Project→초대 순서와 수락의 초대→Project FK 잠금 순서가 엇갈려 교착 롤백이 발생하면 해당 요청은 생성 실패로 처리하고 외부 발송하지 않는다. UI에서 Revoke를 잠그는 것만으로 서버 경합이 해결됐다고 보지 않는다.
...
- Resend 완료 후 실행한 행이 남아 있으면 잠금 해제 뒤 그 Resend 버튼으로, 행이 교체되거나 사라지면 목록 갱신 뒤에도 남는 Pending 카드 제목으로 포커스를 돌린다. 메일 거부·unknown도 발급 후에는 행이 교체되므로 성공 여부만으로 착지점을 고르지 않는다.
- 설정이 없는 상태는 “Email is unavailable right now” 안내다. 수동 링크 폼으로 전환하지 않는다.

수정본의 치수·문구는 아래만 구현 시 보정한다. 새 상태나 디자인 재작업은 필요 없다.

- 핸드오프의 Pending 버튼 높이32는 기존 멤버 행 규칙에 맞춰 **Resend/Revoke 모두36**으로 보정한다. 기존 Button 기본 크기를 쓰며 새 크기 변형을 만들지 않는다.
...
초대 모달의 disabled 포커스 복귀 회귀도 보호한다. `components/__tests__/invite-modal.test.tsx`의 `installFocusFixup`을 유지하고 Pending 검증에도 같은 보정 방식을 적용한다. 복귀 호출을 제거하면 테스트가 실패하는지 확인하고, 실제 브라우저에서 pending 해제·행 교체 뒤 착지점을 확인한다. jsdom에 클릭 당시 포커스가 남아 있는 것만으로 복귀 성공을 판정하지 않는다.
