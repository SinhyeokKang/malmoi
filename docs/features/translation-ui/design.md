...
⚠️ **T6(실물 검증)은 `/ship` 밖이다** — 대상 리포의 워크플로가 프로덕션 `/api/push`를 찌르므로 dev에서 검증할 수 없다(`/merge` 뒤 별도).
⚠️ **그 순서가 "409 → 통과"가 아니다** (2026-09-09 정정): 선언이 서 있으면 **옛 base와 새 base가 둘 다 통과하는 것이 이 설계의 요지**이므로, 409를 보려면 **선언과 무관한 제3의 값**을 선언 **전에** 보내야 한다. 절차는 `tasks.md` T6의 ①~⑤다.
