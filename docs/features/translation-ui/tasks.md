...
>
> ✅ **2026-09-09에 원인이 나왔고 아래 추측은 절반만 맞았다.** 홉이 비용인 것은 맞았는데 **홉이 비싼 이유**가
> 순차성이 아니라 **거리**였다: `x-vercel-id: icn1::iad1::…` — 엣지는 서울인데 **함수가 `iad1`(워싱턴)에서 돌고
> DB는 도쿄**라 왕복 하나가 태평양을 건넜다(홉당 ~375ms). TTFB가 8~77ms인데 본문이 1.5~3.4초 걸리고 문서가 8KB인
> 것이 그 신호였다 — **TTFB를 서버 시간으로 읽으면 안 된다**(헤더는 먼저 나간다). `vercel.json`에
> `regions: ["hnd1"]`을 박았다(PR #24 → `7b029b8`).
>
> | 무엇 | 전 (LCP) | 후 (FCP, 3회) |
> |---|---|---|
> | 기본 착지 (93 textarea) | 3.30초 | **0.44 · 0.47 · 0.54초** |
> | `?ns=*` (2,721 textarea) | 4.66초 | **1.20 · 1.20초** |
> | `order-check` 24키 | 3.29초 | 0.34 · 0.40 · 0.61초 |
> | `/projects` 목록 | 1.42초 | 0.27 · 0.34 · 0.79초 |
>
> `respEnd`와 FCP가 80ms 차이라 옛 관측("LCP가 DCL과 거의 같다")과 같은 지점을 잰다. **2초 목표는 기본 착지뿐
> 아니라 필터 없는 907키 화면에서도 통과했고, 수단 셋 중 아무것도 쓰지 않았다.** 아래는 그때의 추측이다.
>
