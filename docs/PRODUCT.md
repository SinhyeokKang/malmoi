...
- **계정 설정 — 프로필 편집과 사진** (판정 2026-09-13 · **아직 안 만들었다**). 이름을 **사용자 소유**로
  ...
  아니라 **저장 경로**다. ⚠️ **셸 아바타가 이미지를 안 싣는 것은 누락이 아니라 결정이다**
  (`components/shell/user-menu.tsx` — `publicSession`을 넓히면 모든 요청의 세션 페이로드가 커진다).
  사진을 만들면 **그 결정이 같이 걸린다**: 안 뒤집으면 `/account`는 사진, 셸은 이니셜이라 같은 계정이
  두 얼굴이 된다
