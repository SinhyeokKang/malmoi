...
⚠️ **리포 재적재(Sync)는 그 반대편이고 OWNER 전용이다** (판정 2026-09-15 · **아직 안 만들었다** —
`docs/features/sync-repository/`). 리포의 로케일 파일을 다시 읽어 **번역을 리포 값으로 덮는 동작**이라
(`applyPush`의 `"updatedBy" = NULL`), 지금까지 CI만 열던 **편집 손실 창을 사람이 버튼으로 연다.**
**손실되는 편집의 당사자(EDITOR)가 그 창을 스스로 여는 경로를 만들지 않는다** — 그래서 Publish와
방향이 갈린다. 인가는 `project:settings`이고 넷째 permission을 만들지 않는다.
⚠️ **되돌리기를 함께 만들지 않는다**: 옛 DB 값과 새 리포 값 중 고르는 코드가 곧 병합 로직이고
ARCHITECTURE §0 불변식 2와 정면 충돌한다. **방어선은 확인 Dialog 하나다.**
...
- **계정 설정 — 프로필 편집과 사진** (판정 2026-09-13 · **아직 안 만들었다**). 이름을 **사용자 소유**로
...
- **로그인 수단 연결 — `/account`의 [Connect]** (판정 2026-09-13 · **아직 안 만들었다**). §4.3 ④가
...
아니라 **그 조건을 갖춘 경로가 없었던 것**이다. 판정은 §4.1로 올라갔고 **아직 안 만들었다** —
`linkAccount`의 거부(sec-audit-2 #31)는 그대로 서 있다.
...
⚠️ **`/account` 행은 지금 있는 것만 적는다.** §4.1이 그 화면에 항목 둘을 더 얹기로 판정했지만
(프로필 편집·사진 · 로그인 수단 [Connect]) **아직 안 만들었고**, 이 표의 ✅는 프로덕션에 선 것을
뜻한다 — 앞서 적으면 그것을 믿은 사람이 의심해야 할 곳을 의심하지 않는다.
