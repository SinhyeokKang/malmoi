...
- **UI를 여기서 시작한다** — 프로젝트 생성·멤버 관리 화면이 새로 필요하고, 편집 화면도 그때 제대로 만든다 (⚠️ **멤버 관리 화면과 편집 화면 재작성은 SAAS §8 6단계에 남아 있다**)
