...
base 브랜치 푸시 시 GitHub Actions에서 리포의 로케일 파일을 올린다. **base는 `Project.baseBranch`가 정한다** — PoC 대상(bugshot-2)에서는 `dev`였다 (2026-09-01 결정 — 대상 리포 bugshot-2의 실제 작업 브랜치이고, `main`은 보호 브랜치라 PR 머지 흐름이 한 겹 더 생긴다). pull의 base도 같은 브랜치여야 어긋나지 않는다 (§3.3).
...
> **⚠️ 되돌아간 값에 편집자 이름이 남았다 — 2026-09-08에 닫혔다.** push의 `DO UPDATE`가 `updatedBy`를 건드리지 않아, B-3b 직후 편집 UI는 그 셀을 "리포 값 — 편집자 이름"으로 보여줬다 (`CellMeta` — 구 `app/(edit)/keys/page.tsx`, 현 `app/(edit)/projects/[slug]/translations/page.tsx`). **그 사람의 편집은 이미 사라졌는데 화면은 남아 있다고 말했다.** 편집 UI가 동결이라(§8.3) 미뤄 뒀던 것을 SaaS 6a가 답했다: **push가 그 컬럼을 비운다**(`"updatedBy" = NULL`) — 덮인 값의 저자는 리포다. §10에서 그 항목이 닫혔다.
...
6. `POST /api/push` (Bearer `PUSH_TOKEN`), 페이로드에 `projectSlug`·`commitSha`·`commitAt` 포함
   ...
   **`projectSlug`를 페이로드가 싣고 서버가 대조한다** (2026-08-31 결정). PoC 때는 서버 env 하나가 대상 프로젝트를 정했고 그것과 다르면 **409**로 거부했다 — 대상 지정을 서버 env에만 맡기면 리포가 둘 붙는 순간 한쪽 페이로드가 남의 프로젝트에 적용되고, `toDelete`가 없고 FK가 `RESTRICT`라 삽입된 이물 키를 지울 수 없다. 프로젝트별 토큰으로 가르는 방법도 검토했지만 시크릿이 프로젝트 수만큼 늘고 토큰↔프로젝트 매핑을 어딘가 둬야 해서 PoC엔 과했다. ⚠️ **SaaS 5단계가 그 대가를 치렀다** (2026-09-07): 지금은 `sha256(Bearer)` → `Project.pushTokenHash` 조회가 프로젝트를 정하고 페이로드 slug는 **그 뒤에** 대조된다 (SAAS §7.8 · ARCHITECTURE §5.5.5).
...
- 필터 3개: 미번역 / 검토필요 / orphaned — **PoC에서는 동결로 미착수**했다 (§8.3). SaaS 6a ship 3(2026-09-08)이 그 화면을 재작성하며 둘(미번역·검토필요)을 만들었고 orphaned는 필터가 아니라 배지로 남았다 (SAAS §8 6단계). ⚠️ **8-4가 `?state=`를 폐기했다** (2026-09-11) — 상태로 좁히는 대신 pending을 위로 정렬한다. 지금 필터는 네임스페이스·로케일·검색 셋이다
...
⚠️ **8단계 8-4(2026-09-11)가 축을 뒤집었다** — 키 = 그룹, 로케일 = 행이고 `<table>`이 아니다. 근거는 `docs/features/ui-rework/translations/`다.
...
"최상위 키가 하나이고 그것이 로케일처럼 보이면 루트 키"로 관측해 루트 키를 벗기기만 하고 **계약에 싣지 않으며**(`rootKeyedByPath` 필드는 그 뒤 삭제됐다 — `write`가 그 값을 안 믿었다),
...
**MVP에서 비범위, SaaS 단계에서 착수**(2026-09-03 로드맵으로 승격 — §8.4): 테넌트별 인증·인가(멤버십·역할), 프로젝트 생성, 프로젝트당 복수 멤버, 프로젝트 전환 UI. MVP 동안 인증은 `AUTH_ALLOWED_LOGINS`(허용 GitHub 핸들 목록) 하나로 단일 테넌트로 **남았고**, 운영 대상은 서버 env 하나가 가리키는 프로젝트 **하나였다.** ⚠️ **둘 다 지났다** — 허용 목록은 2026-09-06에 삭제됐고(인가는 `ProjectMember`), 편집 경로는 URL slug 멀티테넌트다. **그 env를 읽는 코드도 더 없다** (2026-09-07) — `/api/push`는 프로젝트별 토큰(`Project.pushTokenHash`)으로, `/api/pull`은 준비된 전 프로젝트 순회로 옮겨갔다 (SAAS §7.8·§8 5단계).
...
**4단계**가 이미 만들었다. 다만 승격된 것은 **연결**이지 **설치**가 아니다 — App을 리포에 설치하는
것은 GitHub의 화면이 하고(우리가 대신할 수 없다), 우리가 만든 것은 "이 사람이 그 설치를 볼 수 있는가"를
확인해 `Project.installationId`를 채우는 경로다. 그래서 표현도 "설치 플로"가 아니라 "연결"이다.

**push 라우팅은 단일 테넌트 가정에서 벗어나 있다** (2026-08-31 해소) — 페이로드가 `projectSlug`를 싣고 서버가 대조해 다르면 409로 거부한다 (§3.1 6단계, ARCHITECTURE §5.5.5). 리포가 둘 이상 CI를 붙여도 남의 프로젝트에 적용되지 않는다. ⚠️ **대조 대상이 2026-09-07에 바뀌었다** — 서버 env가 아니라 **Bearer 토큰이 정한 프로젝트**이고, 그래서 "서버가 받는 프로젝트는 하나"였던 제약도 함께 사라졌다 (SAAS §7.8).

MVP 범위를 잡으면서 추가로 뺀 것: **편집 UI의 키 추가·삭제, 로케일 추가·삭제**(리포가 정한다 — §3.2. ⚠️ 행 단위 CRUD는 여전히 0건이지만 **적재 트리거는 SaaS 5단계가 UI에도 뒀다** — 첫 적재 버튼이 `Locale`·`StringKey`를 대량 생성한다. "리포가 정한다"는 그대로고 트리거만 CI에서 UI로 늘었다), 스크린샷 첨부, 번역자 노트 필드, draft→reviewed 승인 워크플로(편집자가 한 명이라 오버엔지니어링), push 웹훅 즉시 반영, 번역 메모리·기계번역.
...
> **⚠️ 이 절은 예고이고, 정본은 [SAAS.md](./SAAS.md)로 넘어갔다** (2026-09-05). 아래 목록은 MVP를 닫을
> 때 내다본 것이고, 실제 범위·순서·설계 결정은 그 문서가 든다 — 특히 **이메일 로그인과 push 웹훅을
> 1차에서 뺀 판정**(SAAS.md §4.3)은 여기에 없다. 이 문서(MVP.md)는 PoC 스펙으로 닫혔다.
...
남은 것 — 붙일 것 (**아래 넷은 당시 내다본 목록이고, 앞의 셋은 SaaS 2·5단계가 닫았다** — 2026-09-07):

- **인증·인가** — 테넌트별 멤버십·역할. `AUTH_ALLOWED_LOGINS` 단일 목록을 대체한다 (✅ SaaS 2단계, 2026-09-06 — 그 목록은 `lib/auth/allow.ts`째 삭제됐다)
- **프로젝트 생성** — 리포 하나가 프로젝트 하나. 당시엔 `Project` 행을 손으로 만들었다 (✅ SaaS 5단계, 2026-09-07 — `/projects/new`)
- **프로젝트당 복수 멤버** (✅ SaaS 2단계 — 초대·역할 변경)
- **UI를 여기서 시작한다** — 프로젝트 생성·멤버 관리 화면이 새로 필요하고, 편집 화면도 그때 제대로 만든다 (⚠️ **멤버 관리 화면과 편집 화면 재작성은 SAAS §8 6단계가 2026-09-09에 프로덕션까지 끝냈다**)
