...
3. **어댑터의 writer로** 로케일별 파일 결정적 생성 (§4) — 읽어온 포맷 그대로
   ...
   ⚠️ **수술적 치환 어댑터(`ts-dict`·`yaml-catalog`·`code-dict`)는 원본 내용을 필요로 한다.** 그 프로젝트에서는 blob SHA만으로 끝나지 않고 로케일 파일의 blob **내용**을 받아야 한다(파일당 API 호출 1회). **2026-09-04부터 재생성 어댑터도 blob 내용을 받는다** — 표현(들여쓰기)을 원본에서 읽기 때문이다(§4.1). 그래서 **"내용을 받는가"에 어댑터별 분기가 더 이상 없다.** 갈리는 것은 **원본이 없을 때**뿐이다: 수술적은 파일을 안 만들고(치환 대상이 없다), 재생성은 기본값으로 계속 만든다(신규 로케일 파일). `layout`으로 가르면 `per-locale` + 수술적 조합(YAML·코드 딕셔너리)이 그 판정을 틀린다.
...
- 들여쓰기: **원본 파일의 폭을 따른다.** 원본이 없으면(신규 로케일 파일) **2칸**이다
  - 2026-09-04 개정. 전엔 항상 2칸이었고, 그게 학습 코퍼스의 **최대 잔여 diff 원인**이었다 —
    4칸·탭 파일 30개에서 값 편집이 0건이어도 모든 줄이 바뀌었다 (`ADAPTER-COVERAGE.md` §14)
