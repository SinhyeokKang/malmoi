...
- **영역**: `lib/pull/payload.ts` · `lib/pull/run.ts` · `.github/actions/malmoi-i18n-push/action.yml` · `lib/onboarding/workflow.ts` · `docs/ACTIONS.md` · `.claude/commands/l10n-roundtrip.md`
- **증상**: 대상 리포에서 번역 PR을 "Create a merge commit"으로 머지하면 CI push가 스킵되지 않고 돌아, DB를 Publish 시점 값으로 덮었다. 그 사이 저장한 번역은 사라진다(strict push의 손실 창이 **우리 자신의 PR 머지로** 열린다). 2026-09-17 `/audit`가 정적으로 찾았고(launch-readiness L1.2), 실물은 `malmoi-test-org/bugshot-i18n-test` PR #1을 `--merge`로 머지해 재현·수정 확인했다(run 35231174347 → job skipped).
- **근본 원인**: 루프 가드 셋(action의 bash `*"[skip-malmoi-i18n]"*` · 워크플로 템플릿의 `contains(github.event.head_commit.message, …)` · ACTIONS.md 예시)이 전부 **`head_commit.message` 하나**를 보는데, 마커는 **스냅샷 커밋 메시지에만** 있었다. squash(기본 메시지)·rebase는 그 커밋 메시지가 base에 실리지만, **merge commit의 `head_commit.message`는 GitHub이 새로 만든 `Merge pull request #N from …\n\n<PR 제목>`**이라 우리 커밋 메시지가 한 글자도 안 실린다. "우리가 만든 문자열이 대상 리포의 head에 그대로 남는다"는 전제가 **머지 방식에 따라 거짓**이었고, PR 제목(`malmoi-i18n: sync translations`)에는 마커가 없었다. ⚠️ **실물 검증 스킬이 이것을 원리적으로 못 봤다** — `/l10n-roundtrip`이 `--squash`만 돌았고, 4단계 확인("dev head 메시지에 마커")도 squash 결과에서만 참이었다. 한 경로만 밟는 검증은 그 경로의 성공을 다른 경로의 성공으로 읽게 한다.
- **그물**: 놓친 것 — `pnpm test`(마커 생산은 `buildCommitPayload` 단위 테스트가 봤지만 소비자가 어느 필드를 읽는지는 아무 테스트도 안 물었다) · `/l10n-roundtrip`(단일 머지 방식) · `docs/ACTIONS.md`(머지 방식을 한 번도 말한 적이 없어 squash 의존이 무문서였다). 잡은 것 — `/audit` launch 렌즈의 정적 읽기. 수정 뒤 `lib/pull/__tests__/skip-marker.test.ts`가 **생산자(`PR_TITLE`·커밋 메시지)를 GitHub이 만드는 머지 커밋 메시지 모양 다섯(merge commit · squash 기본 · squash PR 제목 · rebase · 사람 커밋)에 넣어** 같은 부분 문자열 판정으로 대조하고, 소비자 셋이 같은 리터럴을 드는지 텍스트로 센다. `run.test.ts`는 생성 PR 제목의 마커와 **재사용 PR 제목에 마커가 없으면 덧붙이고(원래 제목 유지) 있으면 PATCH 0회**를 짝으로 고정한다.
- **재발 방지**: 마커는 **커밋 메시지와 PR 제목 둘 다**에 든다(`PR_TITLE`, ARCHITECTURE §3). 검사 쪽(둘째 부모 조회)을 고치는 안은 러너 `fetch-depth`와 `malmoi-i18n-push-v1` 태그 릴리스를 부르므로 버렸다 — 생산자를 고치면 대상 리포·action 변경이 0이다. grep: `rg --hidden -n "head_commit" .github lib docs --glob '!docs/POSTMORTEM.md' --glob '!docs/features/**'` → 소비자는 셋(`action.yml:82`·`workflow.ts:143`·`ACTIONS.md:58`)이어야 하고 그 셋을 `skip-marker.test.ts`가 리터럴로 센다(⚠️ `.github`은 숨김 디렉터리라 `--hidden` 없이는 rg가 건너뛴다). **머지 방식 의존 검증**: `rg -n "gh pr merge" .claude/commands` → 폐기용 리포를 대상으로 하는 스킬은 `--<squash|merge|rebase>`로 방식을 인자로 받아야 한다(`merge.md:61`의 `--squash`는 우리 리포의 `/merge` 정책이라 예외). 더 넓은 교훈: **GitHub이 재조립하는 필드**(`head_commit.message`·squash 기본 메시지·PR 본문 템플릿)에 우리 문자열이 실린다고 가정하는 코드는, 그 필드를 만드는 **모든 경로**(머지 방식·리포 설정)에서 대조한다.

### 2026-09-18 — 관계 필터 count가 대량 적재 직후 5.5초였다 (Prisma LEFT JOIN × 낡은 통계)

- **영역**: `lib/protection/where.ts`(`pendingWhere`·`countPending`·`loadPendingEdits`) · `lib/push/apply.ts` · `app/api/push/route.ts` · `lib/pull/load.ts` · `lib/keys/query.ts`
- **증상**: sync-edit-protection 배포 B 구현 중 `lib/keys/__tests__/repository-import.integration.ts`의 1446키 POST 실측이 30초 타임아웃에 걸렸다. 기준선은 cold 743ms / warm 240ms였는데 cold 6.4초 / warm **19.5초**였다. 새로 넣은 미전달 편집 count(`translation.count({ where: pendingWhere(projectId) })`) 한 문장이 격리 PG 8,676행에서 **5.5초**였다.
...
### 2026-09-24 — 로케일 1개 리포가 온보딩 ②에서 사유 없이 막혔다 (고친 문구가 화면에 닿지 않았다) ([malmoi#99](https://github.com/SinhyeokKang/malmoi/issues/99))

- **영역**: `components/onboarding/new-project.tsx`(탐지·수동 확인 결과 처리) · `lib/onboarding/confirm.ts` `planConfirmedFormat` · `messages/en.tsx` `steps.files.emptyDescription`
- **증상**: 번역 파일이 한 언어뿐인 리포를 고르면 ②가 "malmoi didn't find any on …"(파일은 있다)로 열리고, 수동 지정을 넣으면 `confirmManualFormat`이 불린 뒤에도 "Nothing to preview yet"과 꺼진 Next만 남았다 — 사유 0. 경로를 잘못 친 경우도 같은 갈래라 똑같이 무음이었다.
