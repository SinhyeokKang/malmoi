...
| **font-weight** | 최대 600, 기본 400 | **500과 400 둘뿐이다** — 제목·라벨 500, 나머지 400 (⚠️ 2026-09-11에 기본을 300에서 400으로 올렸다: 상한 500은 그대로이고 **하한이 사라졌다**. `font-light`는 소비자 0). ⚠️ **2026-09-26에 600이 돌아왔다 — 랜딩 h1·CTA h2 둘만**(§4 · §6.615) | §4 |
...
- ⚠️ **weight 규칙이 2026-09-10에 전면 교체됐고 2026-09-11에 하한이 되돌아갔다** — **가장 두꺼운 서체가 500이고, 쓰는 단계는 400과 500 둘뿐이다.**
  ...
  **`body`에 `font-weight: 400`이 있고 그것이 기본이다** (2026-09-11 — 전날의 300을 한 단계
  되돌렸다). **600은 랜딩(`/`)의 h1·CTA h2 둘만 쓴다**(2026-09-26 — 아래 안티앨리어싱 줄이 예고한 첫 소비자, §6.615).
  48px 표시 크기에서 500이 가늘게 읽혀서다. 앱 화면은 여전히 400·500 둘이고, **700 이상은 쓰지 않는다.**
...
**내부 링크는 표면이 두 갈래다.** 셸 **안**의 내부 링크(사이드바·목록 행·진행률 행·활동 행)는 밑줄 없이 `text-foreground`/`text-muted-foreground`다 — 행 전체가 눌리는 자리라 색이 아니라 hover가 그것을 말한다. ⚠️ **셸 밖 화면의 텍스트 링크는 `text-blue-600`이다**(로그인 푸터의 Privacy·Docs, `/docs`의 돌아가는 링크, `/privacy`의 본문 링크 — 공개 셸 안이지만 문단 안 글자라 같은 규칙이다, §6.616) — 그 화면들엔 사이드바도 행도 없어서 **링크가 문단 안의 글자 하나**이고, 색이 없으면 눌리는 것인지 알 수단이 밑줄뿐인데 그것을 8-1b가 전역으로 걷었다. 아이콘은 안 붙는다(리포 밖으로 안 나간다).
...
| **커서** | ⚠️ **꺼지지 않은 `button`은 전역으로 `cursor: pointer`다** (2026-09-25 사용자 — `app/globals.css` base). Tailwind v4 preflight가 `button`을 `default`로 되돌려 `Button` 밖의 raw 버튼(`ListItemButton` · 필터 칩의 Radix 트리거)이 화살표였다. `Button` base의 `cursor-pointer`는 그대로 두되 새 버튼이 기억할 필요는 없다. 꺼진 쪽은 `disabled:`·`aria-disabled:cursor-not-allowed` 유틸이 이긴다(유틸 레이어가 base보다 뒤다). 링크(`<a>`)는 브라우저 기본이 이미 pointer다 |
...
| **Table** | 번역·언어·이력 **셋** + 공개 문서(§6.61) **넷**이 쓴다 (⚠️ **2026-09-19에 멤버가 빠졌다** — 그 화면이 카드 + `<ul>`로 갔다, §6.65. 온보딩 ②의 파일 표는 이 수 밖이다 — §6.643. 세는 명령: `grep -rln 'from "@/components/ui/table"' components app | grep -v __tests__ | grep -v ui/table` → **파일 여섯**, 화면 다섯). shadcn `new-york-v4` 기반이고 `TableHeader`·`TableBody`·`TableRow`·`TableHead`·`TableCell` 다섯을 낸다(⚠️ **`TableFooter`·`TableCaption`은 안 들인다** — 소비자 0). ⚠️ **`Th`·`Td`·`Tr`은 별도 구현이 아니라 그 위의 프리셋이다** (2026-09-12) — 번역 화면이 프리미티브를 직접 들고, 공개 문서는 **섞어 쓴다**(열 머리는 `TableHead`, 셀은 `Td` — §6.61이 그 이유를 든다. `/docs`·`/privacy`가 `components/public-doc-table.tsx`의 `DocTable` 하나를 공유한다). 나머지 셋은 프리셋만 쓴다. 프리셋이 **되눌러야 하는 기본값**(`whitespace-nowrap`·`align-middle`·`border-b`·`h-10`·`px-2`)은 각 함수 위에 적혀 있고 `components/__tests__/table-presets.test.ts`가 렌더해서 센다 — 하나가 안 지워지면 긴 사유가 한 줄로 늘어나고 마지막 행 아래에 선이 하나 더 선다. ⚠️ **`scrollable` 기본이 참이고 끄는 곳이 셋이다** — 번역 화면(`PanelBody`가 스크롤을 소유한다, §6.1) · 온보딩 ②의 파일 표(그 `div`가 스크롤을 들어야 `Th`의 `sticky`가 붙는다, §6.643) · 공개 문서(그 `div`가 `role="region"`을 들어야 키보드로 가로 스크롤된다, §6.61). 컨테이너를 하나 더 만들면 스크롤이 중첩된다 |
...
| 긴 실행의 지연 문구 | **`SlowNotice`**(`components/slow-notice.tsx`) — 원격 실행이 **`SLOW_AFTER_MS`(8초)** 를 넘기면 muted `text-xs` 한 줄 `common.slow`가 `role="status"`로 선다 (2026-09-25 audit-ux #23). 자리는 리포 탐지·샘플 로드(`FilesStep` — 온보딩 ②와 [Add sources]가 공유) · 첫 적재(프로젝트 생성의 상태 줄 · [Add sources] 바닥) · Sync(두 호스트의 결과 자리) · Publish 진행 모달이다 | 스켈레톤·스피너만 30초를 넘게 돌면 "멈췄다"와 "도는 중"이 구별되지 않았다. ⚠️ **값이 하나다** — 화면마다 다르면 같은 대기가 다르게 읽힌다. ⚠️ **단계를 주장하지 않는다** — 진행 이벤트 API가 없다. Publish 모달의 단계 셋은 **하는 일의 목록**이고 체크가 넘어가지 않는다(전엔 2.5초·6.5초 타이머가 일어나지 않은 단계를 넘겼다) |
...
헤더 `(8,8) 1904×48` · 사이드바 `(8,64) 240×1008`(⚠️ 2026-09-26에 헤더가 40이 되어 사이드바는 y=56부터다 — 실측 전 계산값) · 콘텐츠 `x=256 w=1328` · 프로젝트 패널 `x=1592 w=320`. ⚠️ **뒤엣것을 지웠으므로 콘텐츠는 이제 `x=256 w=1656`이다**(1920 실측 1655).
...
| 대기 → 고정 | 대기는 맞춤의 **0.8배**로 뜬 "물건"(베젤 12 · radius 24 · `bg-canvas` · `border-border` · `shadow-medium`). 스크롤 `p = scrollTop / stageTop`(easeInOut)로 맞춤까지 커지고 p = 1이 곧 sticky 시작이다. 고정되면 "화면"(radius 12 · `border-border-subtle` · `shadow-low` — `ContentPanel`과 같은 처리). ⚠️ **radius·그림자 값을 트윈하지 않는다** — 레이어 셋의 opacity 교차로 24 → 12가 바뀌어 보인다. **transform·opacity만** 움직인다 |
