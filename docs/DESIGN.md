...
- **트리거 없는 Dialog도 연 자리로 돌아간다** — `DialogContent`가 최근 포커스 기록에서 아직 붙어 있는 가장 최근 요소로 돌려준다(미저장 확인 · Revert · 역할 변경 · 배너의 [Try again] · Sources 상세 안의 중첩 확인). ⚠️ **"열 때의 `activeElement`"로는 못 잡는다** — 안쪽 `autoFocus`가 FocusScope의 mount 이벤트보다 먼저 돌고, Select 옵션에서 여는 Dialog는 그 순간 포커스가 사라질 옵션 위다. ⚠️ 그 요소가 꺼져 있으면 더 거슬러 가지 않는다(착지가 받는다). 호출부의 `onCloseAutoFocus`가 먼저다 — 확정한 Sync는 트리거(`Syncing…`)로 돌아온다.
