...
| Sessions | 본문 `**Sign out** — this device` / `**Sign out everywhere** — all devices, this one included`. 보조 줄은 **둘 다 있고 provider 이름이 없다**(`You'll confirm with the account you sign in with before anything changes.`). ⚠️ **캔버스의 `GitHub will ask you to confirm…`을 행에 쓰지 않는다 — 의도적 이탈이다**: 그 문구는 확인 상대를 아는 갈래에만 서므로 못 고르는 날 아래 행의 보조 줄만 사라져 **두 행 높이가 갈린다**. 수단 카드에서 보조 줄 둘을 다 지운 것과 같은 논거를 같은 화면에서 반대로 적용하지 않는다. provider 이름은 Dialog의 검은 줄이 들고, 그래야 행과 Dialog가 같은 문장을 두 번 내지 않는다. 항목 둘이 **한 리스트**이고 **Sign out이 위**다 — 같은 축(지금 열린 것을 닫는다)이고 흔한 쪽이 아래에 있으면 사용자가 되돌릴 수 없는 쪽을 먼저 읽는다. 행 버튼은 Sign out `default` / Sign out everywhere **`danger`**. 실패는 **카드 Alert**다(항목 안에 넣으면 행 높이가 항목마다 달라진다). ⚠️ **행 버튼 라벨을 건드리지 않는다** — 이미 `Sign out everywhere`이고, `Confirm and sign out everywhere`는 **Dialog 확정 버튼의 폴백**(확인 상대를 못 골랐을 때)이라 다른 자리다 |
