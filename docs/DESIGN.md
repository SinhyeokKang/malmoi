...
| 글리프 | provider면 **브랜드 마크**(무채색 위계 밖), 그 밖은 lucide **회색**(`link-2` · `log-out` · `monitor-smartphone` — 동작을 가리킨다). ⚠️ **캔버스는 그 회색을 `#525252`로 그렸고 구현은 `--muted-foreground`(#737373)다 — 의도적 이탈이다**: §6.2가 "새 raw 색을 늘리지 않는다"이고 neutral-600은 등재돼 있지 않으며, 리포 전체가 회색 아이콘에 그 토큰을 쓴다. 한 화면만 다른 회색을 두지 않는다 |
