...
Logs 필터 **Sources & locales**는 사건 범주이므로 유지한다. 정식 `/design-sync`와 C2의
미관측 브라우저 상태는 Claude Code에 인계하며, 이 절은 그 검증을 완료했다고 주장하지 않는다.
