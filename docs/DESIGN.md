...
헤더 `(8,8) 1904×48` · 사이드바 `(8,64) 240×1008` · 콘텐츠 `x=256 w=1328`. ⚠️ **그 오른쪽의 프로젝트 패널 `x=1592 w=320`은 2026-09-16에 지웠다**(§6.55) — 콘텐츠가 그만큼 넓어진다.
...
⚠️ **지우면서 콘텐츠가 328(320 + gap 8) 넓어졌다** — 1502 뷰포트에서 콘텐츠 패널이 531 → **1239**이고,
Home 카운트 카드가 **기본 화면에서 처음으로 4열**이 됐다(그 `@[672px]` 컨테이너 쿼리는 2026-09-15에
고쳐졌지만 패널 때문에 임계값을 한 번도 못 넘었다 — §6.64).
