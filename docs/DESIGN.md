...
| **검색 비우기**(`Clear search`) | 모달·목록 안은 `default` 버튼 · **`/projects`는 링크**(`text-blue-600`) | ⚠️ **`/projects`만 갈린 채 두었다** — 캔버스(§6.63 `1c`·`1d`)가 그 화면의 출구를 링크로 내렸고 그 화면은 "px 하나까지" 정본이다. 아래 예외 2의 `default`는 그 개정 전 문장이다 |
...
| 안내 상자 | **`Alert`** — 상시 조건은 `info`(닫기 없음), 실패는 `danger` | logs 보관 안내와 이벤트 상세의 노트 셋이 테두리·아이콘을 손으로 그렸다. ⚠️ **상세의 실패 노트가 `danger`라 `role="alert"`를 든다** — 상세를 여는 순간 실패 문장이 읽힌다(실물 확인 대상) |
...
2. **검색 0건은 액션이 둘이다** — [Clear search] `default` + [New project] `primary`. ⚠️ **2026-09-24 기준 `/projects`는 이 형이 아니다** — 캔버스 개정이 출구를 링크 하나로 내렸다(위 "같은 행동은 같은 형이다" 표). "되돌리라"와
...
- [ ] 조건부 클래스가 `cn()`을 지나나 (§8)
- [ ] 임의값(`text-[Npx]`·`size-[Npx]`·`rounded-[Npx]`) 대신 스케일을 썼나 (§4·§5·§6.8) — 셋 다 전수 0건이다
