...
| `neutral-600` (`#525252`) | 무색 배지의 글자(`Setup`·`Pending`) · 빈 상태 카드의 아이콘 칩 | `Badge neutral`의 기본은 foreground이라 호출부에서 내린다 — 프리미티브를 바꾸면 이 화면 밖 배지가 함께 움직인다 |

⚠️ **2026-09-15에 둘이 등재에서 빠졌다** — `amber-700`(Summary `To review`의 글리프)과
`neutral-700`(`#404040`, 그라데이션 면 위 설명문). **소비자가 0이 됐다**: 앞은 Summary 블록이
사라져서, 뒤는 빈 상태가 카드가 되며 그 면이 없어져서다(`grep -rn "text-amber-700\|text-neutral-700"
app components lib` → 0건). 되살아날 자리가 생기면 그때 다시 등재한다.
