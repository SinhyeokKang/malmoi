...
**상세 640** — 확인창 `dialog.tsx`(440)도 온보딩 `modal.tsx`(1024)도 아니다. 440은 **묻고 끝나는** 표면이라 Before/After 두 블록과 소스별 결과가 들어가면 줄바꿈이 무너지고, 1024는 **라우트를 대신하는** 껍데기라 Back/Next 푸터를 든다. 640은 본문 한 줄이 약 70자에서 끊기는 폭이고, 껍데기 값은 1024에서 비율만 줄여 물려받는다(머리 24/24/16 · 제목 18/500 · 닫기 36 ghost · 푸터 위 선 `--divider`).
