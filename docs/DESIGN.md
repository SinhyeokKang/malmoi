...
| ① 막힘 3갈래 | `EmptyState` — 계정 미연결 → `primary` [Connect GitHub] / 설치 없음 → 외부 링크 "Install the app" / 리포 없음 → "Add repositories to the installation". ⚠️ `GITHUB_APP_SLUG`가 없으면 링크가 사라지고 "Ask your administrator…"로 떨어진다. ⚠️ **검색 0건은 넷째 갈래다** — 요구하는 일이 다르다(검색어를 지워라). ⚠️ **넷 다 본문 세로 중앙이다**(2026-09-13 사용자 실물 — `flex flex-1 items-center justify-center`): 껍데기가 `min-h`로 세로를 잡아 두므로 그냥 반환하면 칩·제목·설명이 헤더 바로 아래 뭉치고 그 아래 수백 px이 빈다. **검색 0건은 검색 필드를 위에 남기고** 그 아래 남은 높이의 중앙이라, 중앙을 잡는 것은 단계 루트(`flex-1`)다. ⚠️ **조회 실패(`Alert`)는 중앙이 아니다** — 폭 100% 배너라 빈 상태와 같은 자리에 서면 둘이 같은 부류로 읽힌다. ⚠️ **설치 요청 대기(`?e=install-requested`, 2026-09-18)는 갈래가 아니라 설명 교체다** — 설치 없음·리포 없음의 설명 전체가 `requested` 한 문장이 되고 링크와 "Refresh this page once you're done."이 빠진다(요청자는 설치를 끝낼 수 없다). 다른 설치로 리포 목록이 이미 서면 목록 위 `info` Alert 한 줄이다 — 거부가 아니라 대기라 `danger` 배너로 세우지 않는다 |
