...
## 8. 어댑터 범용성 측정 (백로그 — 6·7단계 완료 후 착수)

**스펙·설계·태스크는 [features/adapter-generality/](./features/adapter-generality/)에 있다.** 여기엔 진입 조건과 완료 판정만 둔다.

- [ ] 오픈소스 리포 30개 이상에 `detect`+`read`를 돌려 `docs/ADAPTER-COVERAGE.md` 작성
  - 진입 조건: 6·7단계 완료 (왕복이 서기 전엔 지표 ④를 실물과 대조할 수 없다)
  - 검증: 지표 4개(탐지 성공률 / **오탐률** / read 에러 유형별 / 왕복 안정성 + 첫 write 변경 줄 비율)가 숫자로 존재
- [ ] 판정 4개 기록 — 지원 선언 포맷 / MVP §4.1 "키 정렬" 개정 필요 여부 / `ts-dict` 자동 탐지 제외 여부 / **SaaS 경로 개폐**(오탐률 30% 초과면 사내 도구 확정)
  - 검증: 네 판정이 근거 숫자와 함께 `docs/ADAPTER-COVERAGE.md`에 적혀 있다
