...
- [~] **원본 포맷 보존** — **들여쓰기 축 완료** (2026-09-04, ADAPTER-COVERAGE §14): 원인 30 → 0,
      20개 리포가 diff 1.0 → 0.000, 목표 초과 31.3% → 14.1%. 스펙·설계·태스크는
      [features/format-preservation/](./features/format-preservation/)에 있고 `/feature-review`
      3인 검수를 반영했다. **남은 것은 태스크 1b(한 줄 컨테이너 4 + 이스케이프 2)와 엔트리 필드
      순서 1이다** — 좁힌 게이트 분모 4개가 전부 그 축이라 완료 조건 ①의 수치 목표는 거기서
      판정한다. 원 서술: 재생성 writer가 원본의 **들여쓰기·한 줄 컨테이너·이스케이프·엔트리
