...
- [ ] **들여쓰기·공백 보존** — 재생성 writer가 원본의 들여쓰기를 따른다. 판정 ②: 2칸 비율이
      **69.0%**(49/71)라 30개 리포에서 순서를 고쳐도 파일 전체가 diff로 남는다. `serialize`의
      "들여쓰기 2칸" 결정성 규칙을 손대야 해서 축이 다르다. 별 `/feature`
...
- [ ] 태스크 6~7 — 재측정 게이트, 실물 확인, 문서 마무리 ⬅️ **다음에 할 일**
