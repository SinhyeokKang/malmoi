...
- [ ] ❌ 재생성 어댑터(`chrome-locales`·`json-catalog`) 실물 pull — **같은 프로젝트로** 두 표면을 다루는 것은 MVP §7 비범위
...
- [~] **원본 포맷 보존** — **들여쓰기·한 줄 컨테이너·이스케이프 완료** (2026-09-04): 들여쓰기는
      ADAPTER-COVERAGE §14가 실측했고(원인 30 → 0, 20개 리포가 diff 1.0 → 0.000, 목표 초과
      31.3% → 14.1%), 한 줄 컨테이너 4 + 이스케이프 2는 태스크 1b(`ccee7ab`)가 닫았다 —
      L2 골든 `key-order-golden.test.ts`의 `COMPACT_CHROME`·`ESCAPED`가 바이트 동일과 고정점을
      단언하고 **9차 실측이 확인했다** (ADAPTER-COVERAGE §15: 원인 4+2 → 0, button-stealer
      0.964 → 0.000, chrome 중앙값 0.0362 → 0.0280, 전체 초과 14.1% → 13.1%). 셋 다
      `DiffCauses`에서 관측치로 내려갔다. ⚠️ **minify된 파일은 범위 밖이다** (HeaderEditor
      1.000 — §15.3). 스펙·설계·태스크는
      [features/format-preservation/](./features/format-preservation/)에 있고 `/feature-review`
      3인 검수를 반영했다. **남은 것은 엔트리 필드 순서 1(chrome `description` 선행 다수결)뿐이고**
      완료 조건 ①의 수치 목표는 거기서 판정한다 (Midnight-Lizard 0.456). 원 서술: 재생성 writer가
      원본의 **들여쓰기·한 줄 컨테이너·이스케이프·엔트리 필드 순서**를 따른다. `serialize`의
      결정성 규칙이 만드는 차이라 **한 기능이 묶어서 다룬다**. 5차 실측 기준 학습 코퍼스의 최대
      잔여 원인이었다: 들여쓰기 **30** + 한 줄 컨테이너 **4** + 이스케이프 **2**
      (`ADAPTER-COVERAGE.md` §11.3·§11.5). 별 `/feature`
