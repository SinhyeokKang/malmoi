...
(`695e441`). 그 뒤 **6b-1**(어댑터 오류 코드화 — `982cb42`)·**6b-2**(멤버 화면 — `a00d380`)가 프로덕션에,
**6b-3**(base branch·기준 로케일 — 2026-09-09)이 dev에 올랐다. 남은 것은 **6b-4 하나**(`/account`를 만들지
말지의 판정)와 **6b-3의 T6**(실물 409 검증 — `/merge` 뒤에 돈다)이다.
...
- [ ] **키 리스트 가상화 — 답은 가상화가 아니었다** (2026-09-08 재측정으로 닫혔다). 관측(2026-09-07)은
  ...
  **가상화도 조회 좁힘도 이 3.3초를 못 줄인다** — 목표 2초는 미달이고 남은 일은 번역 화면 **밖**이다:
  표를 `Suspense`로 감싸 셸을 먼저 그리기 · 순차 DB 왕복 병합 · 폰트 CSS의 렌더 블로킹 해제
  (`docs/features/README.md` 백로그. 착수 전에 어느 왕복이 얼마인지부터 재야 한다)
