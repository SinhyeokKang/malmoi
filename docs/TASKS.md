...
| **B. 데이터 플로우 체크** | 세 흐름이 **끝에서 끝까지** 값을 안 잃는다 | pull만 됨 |
...
- [ ] **B-3. 손실 창 실증** — 이 도구의 핵심 계약이고 **아직 한 번도 관측되지 않았다.**
      §6에서 "사용자 결정으로 범위 밖"이었으나 2026-09-03 범위 재정의로 **B에 들어왔다**
  - **B-3a. 창이 닫히는 쪽** — push → 편집 → pull → **PR 머지** → push. 머지된 리포 값이 곧 DB
    값이므로 strict가 덮어도 **DB가 그대로여야 한다**
    - 검증: 마지막 push 후 `Translation.value`가 편집한 값 그대로. `updatedAt`은 갱신돼도 된다
  - **B-3b. 창이 열려 있는 쪽** — push → 편집 → **머지 전에** push. 스펙이 "그 편집이 사라진다"고
    적어 놓은 대가가 **실제로 그렇게 동작하는지** 본다 (MVP §3.1)
    - 검증: 편집 값이 리포 값으로 되돌아간다. **되돌아가지 않으면 그게 더 나쁘다** — 스펙과
      코드가 갈렸다는 뜻이고, 어느 쪽이 참인지 모르는 상태가 된다
  - ⚠️ **`order-check` 리포로 돌린다** (23키, 실물 확인에서 만든 것). bugshot-2를 쓰면 903키가
    움직이고 prod DB가 곧 dev DB다
  - ⚠️ **B-3b가 데이터를 지우는 실험이다.** 테스트 프로젝트에서만 돌리고, 돌리기 전에
    `ACTIVE_PROJECT_SLUG`가 무엇을 가리키는지 확인한다
...
- [ ] **원본 포맷 보존** — 재생성 writer가 원본의 **들여쓰기·한 줄 컨테이너·이스케이프·엔트리
      필드 순서**를 따른다. `serialize`의 결정성 규칙이 만드는 차이라 **한 기능이 묶어서 다룬다**.
      5차 실측 기준 학습 코퍼스의 최대 잔여 원인이다: 들여쓰기 **30** + 한 줄 컨테이너 **4** +
      이스케이프 **2** (`ADAPTER-COVERAGE.md` §11.3·§11.5). 별 `/feature`
