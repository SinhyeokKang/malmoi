...
- **편집 스칼라의 `range`만 뒤에서 치환한다** (2026-09-16 T12 수정). 문서 전체를 `toString()`으로 재직렬화하지 않는다. 앵커·태그·주석·정렬 공백·미편집 folded scalar는 원본 바이트로 남는다. 블록 헤더의 들여쓰기·주석과 CRLF를 보존하며, chomping은 요청값에 맞춘다. 공백만 있는 값처럼 블록으로 의미를 유지하기 어려운 경우는 인용 표현으로 낸다.
...
- ⚠️ **그 `lineWidth: 0`이 반대 방향의 대가를 만든다** (2026-09-16 실물 왕복 — `i18n-format-check#3`).
  ...
  **2026-09-16 T12 수정**: 노드의 `range`로 편집 스칼라만 치환하고, 누락 키도 기존 맵 끝에 삽입하도록 바꿨다.
  `yaml-catalog.test.ts`가 실제 편집·삽입 시 미편집 바이트 보존과 요청값·반복 결정성·재적용 고정점을 각각 검사한다.
