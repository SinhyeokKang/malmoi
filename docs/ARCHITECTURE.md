...
1. `t()` 인자가 부족하다 (`t(key, source)` 형태여야 한다)
2. `t()` 호출의 1·2번 인자가 문자열 리터럴이 아니다 → `// @l10n-keys a, b` 주석으로 명시 등록 가능. **템플릿 리터럴은 치환이 없어도 거부한다** — 허용하면 "치환이 있는 경우"와 형태로 구분되지 않아 규칙이 흐려진다
3. 같은 키에 서로 다른 원문 — 조용히 하나를 고르면 다음 스캔에서 뒤집혀 `needsReview`가 무의미하게 전파된다
4. 정규식 경로에서만 발견된 키에 대응하는 원문이 없다
5. **키 이름이 `[A-Za-z0-9_@]` 밖이다** — `chrome.i18n`이 그 메시지를 **조용히 무시**해 런타임에 진단 없이 깨진다. `common.ok` 같은 점 표기가 여기서 걸린다

에러는 `path:line`을 들고 있어야 한다 — CI 출력에서 사람이 바로 찾아가야 한다.
...
**`description`의 공급원은 `// @l10n-desc <text>` 주석뿐이다.** §5.1의 래퍼 `t(key, _source, subs?)`에 description 슬롯이 없어서 생긴 구멍을 `@l10n-keys`와 같은 계열의 지시자로 메웠다. 호출 바로 위에서 위로 올라가며 찾고, 주석·빈 줄이 아닌 줄을 만나면 멈춘다(멀리 떨어진 지시자가 엉뚱한 호출에 붙는 걸 막는다).

**`namespace`는 첫 밑줄 앞부분, 없으면 `_root`다.** 위 문자셋 제약 때문에 구분자가 밑줄뿐이다. 빈 문자열 namespace를 만들면 사이드바에 이름 없는 그룹이 생기므로 선행 밑줄만 있는 키도 `_root`다.

**`scanSources`는 I/O가 없다** — 소스 텍스트를 받는 순수 함수고, 파일시스템을 아는 건 `scripts/scan.ts` CLI뿐이다. 그래서 테스트가 가상 소스로 자기완결한다.

**출력은 정렬한다** (키·refs 모두, export와 같은 `<` 비교). 스캔 결과가 push 페이로드라 결정적이지 않으면 서버 쪽 diff가 노이즈가 된다.
