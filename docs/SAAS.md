...
✅ **8-4(번역 화면)와 2026-09-11 폴리싱 라운드**(패널 머리 고정 · 컨트롤 36px · `lib/tone.ts` ·
콘텐츠 상한 1280)**도 프로덕션에 나갔다** (PR [#34](https://github.com/SinhyeokKang/malmoi/pull/34) →
squash `77630c4`). 그 PR에 **8-4 시안 정합 라운드**가 함께 실렸다(같은 날): 시안
...
**dev에 남아 있는 것은 셋이다.** ① **weight 라운드** (2026-09-11): 본문 기본이 300 → **400**이 되어 쓰는
weight가 **400·500 둘뿐**이다. 상한 500은 안 바뀌었고 바뀐 것은 하한이며, `font-light`는 소비자가
0이 됐다(둘은 클래스 삭제로, 상속된 500을 되눌러야 하는 둘은 `font-normal`로).
② **타입 스케일** (2026-09-12): `--text-xs`가 12 → **13px**이라 `@theme`이 덮는 단계가 `base`와 둘이 됐고,
**mono 표면과 크기가 같아졌다** — 두 표면의 구별이 크기가 아니라 글꼴로 넘어갔다 (DESIGN §0·§4.1).
③ **8-5~ 톤앤매너 통일의 계획과 첫 수정 셋** (2026-09-12 — `docs/features/ui-rework/page-patterns/`):
나머지 일곱 화면의 시각 표현을 기준 넷(로그인·목록·번역·셸)에 맞추는 배송 계획이고, 두 해상도
실물 비교에서 나온 **멤버 제목 행 `min-h-9`**(OWNER 전용 버튼이 줄 높이를 정하고 있었다) ·
**수동 기준 로케일 입력의 mono 제거** · **`Dialog` Overlay·Content `z-50`**(관리 표의 sticky `z-10`이
모달을 덮었다)까지 얹혔다. 화면별 본문 재작성은 아직이라 **8-5~는 ⬜ 그대로다.**
...
- [x] ✅ **번역 표의 축이 바뀌었다** (2026-09-11, 8-4). 키가 왼쪽 셀 하나(320)이고 **로케일이 행으로
      ...
      대체**됐다(키 단위로 한 번만 센다). 마크업은 2026-09-12에 **shadcn 기반 `<table>`**로 전환했다. 키별 tbody·rowSpan과
      열 너비·스크롤 계약은 DESIGN §6.1이 든다. `localeProgress`는 안 바뀌었다(Home·언어 화면의
      것이라 축이 다르다)
  - ⚠️ **"행 수가 3배"가 "입력이 3배"는 아니다** — 907키 × 3로케일 = **`<Textarea>` 2,721개**이고(2026-09-12 실측) 그
        수는 축이 바뀌어도 그대로다. 늘어나는 것은 행 래퍼와 로케일 배지이고 입력보다 싸다.
        **국기는 CSS `background-image`라 요소가 0개 는다.** 가상화는 선반영하지 않았다
  - ✅ **`?ns=*` 2초 게이트를 쟀고 통과했다** (2026-09-12 — 로컬 프로덕션 빌드 + dev DB의 `perf-903`,
        bugshot-2 · ts-dict · **907키**, FCP 3회). **FCP가 기본 착지·`?ns=*` 둘 다 0.3초대**다 — 스트리밍이라
        첫 페인트가 셸에서 나고 행 수와 거의 무관하다.
        ⚠️ **게이트가 못 보는 축이 있다** — `?ns=*`는 `<Textarea>` **2,721개** · DOM 노드 34,624 ·
        문서 1.44MB라 **`loadEventEnd`가 3.9초**이고, 로컬이라 네트워크가 0인데도 그렇다.
        **그래도 가상화를 넣지 않았다**: 판정 조건이 게이트 미달인데 통과했고, `?ns=*`는 기본 경로가
        아니라 사용자가 일부러 고르는 전체 보기다(기본 착지는 load까지 1초). 다음에 이 화면이
        느리다는 제보가 오면 **FCP가 아니라 그 `loadEventEnd`부터** 본다
        (`features/ui-rework/translations/tasks.md` 실측 기록)
