...
- [x] ✅ **번역 표의 축이 바뀌었다** (2026-09-11, 8-4). 키가 왼쪽 셀 하나(320)이고 **로케일이 행으로
  ...
  - ⚠️ **`?ns=*` 2초 게이트는 아직 실측 전이다** — 903키 프로젝트가 dev DB에 없어 `push:local` 적재가
...
- [x] ✅ **국기의 폴백 계약이 정해졌고 구현됐다** (2026-09-11, 8-4 — `lib/keys/flag.ts`): 매핑이
      ...
      ⚠️ **지금 `FLAG_INVENTORY`가 비어 있다** — 전 로케일이 폴백(코드만)으로 서고 그것이 정상 상태다.
      에셋이 오면 **목록 · `public/flags/` · `globals.css` 규칙 셋이 함께** 늘어야 하고
      `lib/keys/__tests__/flag-assets.test.ts`가 그 셋을 대조한다
