...
✅ **8-1a(라우트 이관)가 dev에 나갔다** (2026-09-10). 시안 적용 전에 **`/signin`을 독립 라우트로
가르는 것**만 먼저 했다 — 랜딩 자리를 비우고, 로그인 목적지 아홉을 `lib/routes.ts` 하나로 모으고,
`/privacy`·`/docs` placeholder를 세웠다. 화면은 아직 옛 디자인이다.
...
우선 · slate 유지 · **피드백은 토스트** · 브랜드 로고 인라인 SVG)은 `docs/features/ui-rework/README.md`가
