...
>
> **§10은 4차(2026-09-02)다** — 어댑터를 안 건드리고 **지표만 늘려** 학습·홀드아웃을 다시 돌렸다.
> 키 순서 보존(`docs/features/key-order-preservation/`)의 설계 판정 3개가 거기 있다.
...
### 판정 ② MVP §4.1 "키 정렬" — **개정 확정, 별 `/feature`로 분리됨** (2026-09-02 해소)
...
| 2 | ~~**"원본 키 순서 보존" 모드**~~ → `docs/features/key-order-preservation/` (진행 중, §10) | 판정 ② — `json-catalog` diff 중앙값 0.784 |
