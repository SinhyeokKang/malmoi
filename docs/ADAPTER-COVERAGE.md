...
| `adapter-threw` | — | 어댑터가 예외를 던졌다(포맷 가정이 깨진 자리). 2026-09-02부터 `ReadErrorKind`에 있고 **위 건수는 그 이전 값이라 분모가 없다** — 재측정 필요 |
| `other` | — | 위 어디에도 안 맞는 것. 같은 라운드에 추가됐고 같은 이유로 분모가 없다 — 재측정 필요 |
