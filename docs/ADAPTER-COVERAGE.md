...
⚠️ **이 문서의 탐지 지표는 "전 후보에 probe를 먹인" 조건의 값이다** (`lib/survey/select.ts`가 **`MAX_SHAPE_GROUPS` 8로 모양 그룹을 먼저 자른 뒤** `FILE_BUDGET` 1200까지 로컬 clone 파일을 넣는다 — ⚠️ **측정 코퍼스에서도 일부 후보는 내용이 안 실린다**, 그만큼 아래 경계가 덜 날카롭다). **프로덕션 온보딩은 상위 5+2만 probe한다** (`lib/onboarding/detect.ts`의 `PROBE_LIMITS`, blob ≤21) — 그리고 `chrome-locales`·`json-catalog`·`yaml-catalog`은 **probe를 주는데 그 경로의 내용이 안 오면** `verifySamples`가 `false`라 **미검증 = 탈락**이다. ⚠️ **`probe` 자체가 없는 호출은 다르다** — 셋 다 `if (probe && !verify(…)) continue`라 검증을 통째로 건너뛰고 후보가 **미검증인 채로 산다**(온보딩 1패스가 그 경로다). 탈락은 2패스에서만 일어나고, 그래서 **6순위 이하 후보는 프로덕션에서 조용히 사라지는데 실측 코퍼스에서는 살아 있었다** — §0의 "discourse의 정답이 5순위였다"가 정확히 그 경계선이다. (`code-dict`만 반대다 — `probe`가 없으면 아예 빈 배열이다.)

**`null` 리프는 에러가 아니다** (2차에서 정정). jsxc 한 리포가 이것만으로 5,099건을 냈는데, 그 리포는
미번역 키를 `null`로 두는 관례다 — 빈 문자열과 같은 취급이 맞고, 에러로 세면 남의 CI를 우리 관례로
실패시킨다.

⚠️ **이 표가 못 보는 부류가 하나 더 있고 그 값은 여기에 없다.** 코드 어댑터의 **부분 읽기**
(`readKeyCount < literalCount / 2` — 에러도 무증상 skip도 아닌데 읽힌 키가 파일의 문자열 리터럴 수에
크게 못 미치는 경우)는 `lib/survey/summarize.ts`의 `partialReadRepos`가 세는데, **CLI 사람용 출력에
안 찍히고 `--json`에만 실린다.** 그래서 회차를 돌려도 요약 표에는 안 나타난다 — 값을 보려면
`--out`으로 받은 JSON을 열어야 한다. 지표를 넣고 배선을 안 한 모양이고(§16.1과 같은 계열), 다음
회차에서 이 행을 채우려면 그쪽부터 손대야 한다.
...
**"변경 줄 비율"의 정의**: `1 − 2·LCS / (원본 줄 수 + 출력 줄 수)` (`lib/survey/diff.ts`의
`roundtripDiffRatio`). **"바뀐 줄 / 전체 줄"이 아니다** — 그렇게 세면 삽입과 삭제가 비대칭이라 파일이
길어질 때 값이 흔들린다. 이 식이라야 동일 → `0`, 한 줄 변경 → `1/n`, 전면 재정렬 → `1 − 1/n`이
정확히 떨어진다. 이 문서의 모든 diff 숫자가 이 식의 값이다.

⚠️ **예산을 넘은 리포는 근사값이고, 근사는 값을 작게 만든다.** 매칭 쌍이 `MATCH_BUDGET`
(5,000,000)을 넘으면 `roundtripDiffRatio`가 **순서 비민감 근사**로 떨어져 비율을 **과소평가**하고
`changedHunks`는 `undefined`를 낸다. 이 지표는 **작을수록 좋다고 읽히므로 과소평가가 곧 거짓
안심이다** — 근사된 리포가 요약 표에서는 평범한 값과 구별되지 않는다. 구별 신호는 셋이다:
`RepoSurvey.diffApproximate` · `metrics.diff.approximateRepos` · 리포별 표의 비고 열 `diff 근사`.
**diff 숫자를 인용할 때는 그 리포가 근사였는지 먼저 본다.**

## 2. 어댑터별

⚠️ **아래 표는 2차 시점의 출력이라 열이 여덟이고, 지금 생성기는 아홉을 낸다** —
`lib/survey/summarize.ts`가 `diff 중앙값` 뒤에 **`비-base diff`**를 더했다(§10.4가 그 값을 따로
서술한다). **여기 값을 손으로 채우지 않는다** — 다음 회차에 표를 새로 붙일 때 열이 늘어난다.
...
`lib/adapters/shared.ts`)와 탐지 검증 헬퍼(`verify`·`localeFromPath` — 이 둘은 **`chrome-locales.ts`에
있고 `json-catalog`이 거기서 가져다 쓴다**. 앞의 것은 `shared.verifySamples`의 얇은 래퍼다)다.
⚠️ **두 어댑터가 read/write를 공유한다는 뜻이 아니다**: `json-catalog`은
...
⚠️ **이 표는 열이 열셋이고, 지금 생성기는 열다섯을 낸다** — `lib/survey/summarize.ts`가 `diff`와
`비고` 사이에 **`순서 일치`·`들여쓰기`**를 더했다(§10.1·§10.2의 판정이 그 두 값 위에 선다).
**여기 값을 손으로 채우지 않는다** — 다음 회차에 표를 새로 붙일 때 열이 늘어난다. §9도 같다.
...
| `MAX_PER_SHAPE_GROUP` | 12 | **접두사·로케일 디렉터리** 모양 그룹 하나에서 고르는 파일 수 |
| `LOCALE_CODE_PER_DIR` | 12 | **코드 딕셔너리 디렉터리** 하나에서 고르는 로케일 파일 수 (`.ts`·`.js` 중 이름이 로케일인 것이 2개 이상인 디렉터리 — `code-dict` 후보다) |
| `TS_PER_DIR` | 8 | 그 밖의 TS/JS 디렉터리에서 고르는 파일 수 (`ts-dict` 형태 probe) |

⚠️ **맨 로케일 파일 모양에는 파일 수 상한이 없다.** 크롬 `_locales`·`{dir}/{locale}.json`·
`{dir}/{locale}.y(a)ml` 그룹은 로케일이 2개 이상이면 **그 디렉터리의 파일을 전부** 담고, 자르는 것은
`FILE_BUDGET` 하나다. 12 상한은 **접두사 붙은 파일명·로케일 디렉터리** 모양에만 걸린다.

⚠️ **전에 이 자리가 "로케일이 12개를 넘는 리포(mastodon 106, jellyfin-web 107)에서 일치율·비-base
diff는 12개 표본 값이다"라고 적었는데 거짓이었다** — 둘 다 맨 로케일 파일 모양이라 상한이 안 걸린다.
**문서 자기 표가 그 증거다**: §8이 그 두 리포의 로케일을 107·106으로 적고 있다(12개만 담았다면
거기가 12여야 한다). 실제 한계는 `FILE_BUDGET` 1200이고, 그것에 걸린 리포는 `truncated`로 표시된다.
...
## 22. 16차 측정 — sec-audit-2 글롭 DP 전환 뒤 (2026-09-10)

`matchGlobPaths`의 정규식 역추적을 DP로 교체한 뒤 학습 109개·홀드아웃 20개를 모두 재측정했다.
각각 기존 `repos.txt`/`repos-heldout.txt`와 해당 verdicts 파일을 `pnpm adapter-survey`에 넘겼다.
네트워크·조회 실패는 양쪽 모두 0건이다. 이 측정은 온보딩 다운로드 예산의 허용률 측정이 아니다.

| 지표 | 학습 | 홀드아웃 |
|---|---|---|
| 지원 포맷 탐지 | 100/101 | 16/17 |
| 오탐 | 0/100 | 1/16 |
| 왕복 의미 동일 | 99/100 | 16/16 |
| 바이트 고정점 | 100/100 | 16/16 |

위 지표는 15차와 같다. 기존의 의미 왕복 실패·홀드아웃 오탐은 남아 있으며 새 일반화 성능을
입증한 것은 아니다. 공격적 긴 경로의 매칭 실패는 `glob-linear.test.ts`가 별도로 검사한다.

## 23. 17차 측정 — 8단계 UI 재작성 뒤 (2026-09-11)

`/doc-check`이 요약 함수의 **표 열 수**가 문서와 어긋난 것을 잡아서 돌렸다. ⚠️ **이 회차는 어댑터·
survey 코드를 한 줄도 안 건드린 상태에서 잰 것이다** — 8단계는 UI 배송이고 `lib/adapters/**`·
`lib/survey/**` 변경이 0이다. 그래서 이 숫자가 답하는 것은 **"UI 라운드가 어댑터를 안 건드렸다"**
하나뿐이고, 일반화도 회귀 수정의 효과도 아니다.
...
| 왕복 의미 동일 | 99/100 | 16/16 |
| 바이트 고정점 | 100/100 (100%) | 16/16 (100%) |
| 정답 순위 분포 | `{1: 100, 없음: 1}` | `{1: 15, 5: 1, 없음: 1}` |

**16차와 한 칸도 다르지 않다.** 어댑터별로도 같다 — `chrome-locales` 33리포 diff 중앙값 0.025
(초과 8/33) · `code-dict` 11리포 0.000 · `json-catalog` 38리포 0.000(초과 4/37) · `yaml-catalog`
18리포 0.000.

⚠️ **문서가 적고 있던 표 열 수가 실제와 달랐다** — §2의 어댑터별 표는 코드가 **아홉 열**(`비-base
diff`가 `diff 중앙값` 뒤)을 내고, §8·§9의 리포별 표는 **열다섯 열**(`순서 일치`·`들여쓰기`가 `diff`와
`비고` 사이)을 낸다. 그것이 이 회차를 돌린 이유이고, **위 표는 그 열까지 확인한 값이다.**

⚠️ **홀드아웃은 여전히 학습 코퍼스다** (§0 3차 — 이번이 여섯 번째 회차다). 6.3%는 "회귀가 없다"만
말한다. 일반화를 다시 재려면 **처음 보는 리포**가 필요하고, 그 코퍼스를 새로 뜨는 것은 아직 안 했다.
