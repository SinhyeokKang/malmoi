...
- **`docs/features/<slug>/tasks.md` 확인**(있으면) — 이 작업이 어느 태스크의 완료 조건에 해당하는지 확인한다. **`🔒` 표시가 붙은 항목을 건드리면 중단하고 결정을 받는다.** 목록에 없는 작업이면 왜 필요한지 보고에 적는다(범위 확장일 수 있다).
- **`docs/PRODUCT.md` 확인** — 구현이 제품 판정과 어긋나면 코드가 아니라 그 문서를 먼저 고칠 문제일 수 있다.
...
- **`pnpm build` 금지** — 타입 확인은 `pnpm typecheck`. 빌드 게이트는 `/push` 1단계에 있다(RSC 경계는 거기서 잡는다). 구현 중 매번 돌리면 루프가 느려지고, 배포될 HEAD가 아닌 중간 상태를 검증하게 된다.
