...
⚠️ **`Page` 행은 freeform(직접 작성) 모드에서 계속 갱신된다 — 고정된 값이 아니다** (2026-09-09에 고쳤다. 그 전 서술은 "draft 생성 시점에 고정"이었고 실측이 반대였다: 패널을 리마운트하지 않은 채 **대상 탭만** 이동시켰는데 그 행이 따라 바뀌었고 `Captured` 시각은 그대로였다). **BugShot이 의도한 동작이다** — freeform에는 캡처 순간이 없어 어느 시점으로 고정해도 애매하므로 바인딩된 탭의 현재 URL을 읽는다. `read-only`인 것은 여전히 맞다(`input.readOnly === true` — bugshot-2#228): 고정된 것은 **편집 가능성이지 값이 아니다.**

**그래서 이 행의 쓸모가 둘로 갈린다:**

| 무엇에 쓰나 | 유효한가 |
|---|---|
| **바인딩 검증** (패널이 내 탭을 잡았나) | ✅ 유효하다 — 지금 이 순간의 바인딩을 그대로 보여준다. 대상 탭을 **아는 URL에 둔 채** 읽는다 |
| **결함을 관측한 페이지의 근거** | ❌ 못 쓴다 |

⚠️ **위험은 거짓 불일치가 아니라 거짓 일치다.** 그 행은 내가 지금 보고 있는 탭을 따라오므로 **어디를 보고 있어도 "맞는" 값이 나온다** — 그래서 결함이 다른 라우트에서 관측됐다는 사실을 조용히 지운다. **재현 절차에 실제 라우트를 반드시 적는다**(캡처를 붙였어도 그렇다). 캡처 순간을 특정해야 하는 결함이면 **영역 캡처를 그 화면에서 떠서** 본문에 넣는 것이 유일한 근거다.
...
| `Page` 행의 수명 | ⚠️ **고정이 아니다** — freeform에서 바인딩된 탭을 따라 갱신된다 (2026-09-09 실측·설계 확인). §4의 표를 본다 |
