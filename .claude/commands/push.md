...
>
> **프로덕션 앞의 게이트는 `/merge`의 PR CI로 옮겨갔다** (2026-09-04 브랜치 분리). 브랜치가 하나였을 때는 이 스킬의 1단계가 유일한 방어선이었다. `main`은 브랜치 프로텍션(required check `verify`, 관리자 포함)이 직접 push를 거부한다(2026-09-18).
...
- **개인정보 방침을 거짓으로 만들 수 있는 diff** — `prisma/schema.prisma` · `auth.ts` · `lib/credentials/**` · 새 `fetch(` 호스트 리터럴 · 새 외부 SDK·서비스 env · `cookies().set` → **`messages/en.tsx`의 `publicDocs.privacy`** (2026-09-24, privacy P5). 자동 게이트(`collected.ts` 전수 등재 → typecheck, `policy-gate.test.tsx` → 본문 해시·시행일)가 **원리적으로 못 보는 넷**을 묻는다: ① 같은 데이터의 **새 목적** ② 새 **외부 전송처**(초대 메일의 Resend가 그 예다 — 호스트가 SDK·env 안에 있어 리터럴이 없다) ③ **쿠키·보존 기간**(값이 상수라 "늘었다"를 셀 기준이 없다) ④ 본문의 **내부 모순**. 고치면 `policy-gate.test.tsx`의 `REVISIONS`에 행을 더하고 시행일·`changes` 절을 옮긴다. ⚠️ **`set(` grep은 `@auth/core`가 심는 쿠키(`authjs.csrf-token`)를 못 본다** — 쿠키는 브라우저 실측이 정본이다
