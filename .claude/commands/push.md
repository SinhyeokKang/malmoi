...
- **⚠️ 확인은 `pnpm db:status:prod`로 한다, `db:status`가 아니다** (2026-09-04 dev/prod 분리 뒤). 후자는 **dev**를 본다 — dev엔 적용됐고 prod엔 안 됐는데 "up to date"를 읽으면 그대로 푸시된다. 분리 전에는 인스턴스가 하나여서 `migrate dev`가 이미 프로덕션을 바꿔놨고, 그래서 이 게이트를 잊어도 안 깨졌다. **분리가 만든 새 실패 모드다.**
