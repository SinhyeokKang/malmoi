...
| `actions` | **docs/ACTIONS.md** | 대상 리포 워크플로 예시·`inputs` 표·red 조건 표·경고 조건(열린 번역 PR — 브랜치 이름이 `lib/pull/trigger.ts`의 `syncBranchFor`와 같은가)이 `.github/actions/malmoi-i18n-push/action.yml`과 일치하는가 |
