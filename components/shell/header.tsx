 ...
 *
 * ⚠️ **SAAS §8의 "top bar가 사라진다"는 *지금의* top bar 얘기다.** 시안에는 전폭 48 헤더가 있고,
 * 이 파일이 옛 `top-bar.tsx`를 대체한다 — 상단이 두 벌이 되지 않게 그쪽은 지웠다.
 *
 * ⚠️ **패널이 아니다.** 헤더는 캔버스 위에 그냥 얹힌다(배경·border·그림자 0) — 시안에서 흰 패널은
 * 콘텐츠와 오른쪽 패널 둘뿐이고, 헤더에 배경을 주면 그 대비가 무너진다 (규약 3.5).
 *
 * ⚠️ **breadcrumb이 여기 없다.** 레이아웃은 페이지 props를 못 받으므로 페이지 콘텐츠의 첫 줄이
 * 든다 (`features/translation-ui/design.md` §2). 8-3이 그것을 `[slug]` 레이아웃으로 옮길 자리다.
 */
