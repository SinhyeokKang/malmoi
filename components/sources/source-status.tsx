 ...
 */
export function SourceStatus({ source, now, icon = false }: { source: Parameters<typeof planSurfaceImportStatus>[0]; now: Date; icon?: boolean }) {
  ...
  return <span data-source-status={status.state} className={cn("flex items-center gap-1.5 text-xs", failed ? "text-destructive" : "text-muted-foreground")}>
    {/* 색각 이상에서 정상과 실패가 같은 회색 문장이 되지 않게, 색이 아니라 모양이 상태를 든다. */}
    {icon && failed && <CircleAlert className="size-3.5 shrink-0" aria-hidden />}
    {icon && status.state === "importing" && <LoaderCircle className="size-3.5 shrink-0 animate-spin" aria-hidden />}
