 ...
 *
 * ⚠️ **`components/ui/panel-card.tsx`가 아닌 이유**: 형이 가깝지만(헤더 + 행 목록) 그쪽은 설정·계정의
 * 그릇이라 빈 상태·`<details>`·Meter를 안 든다. 합치면 이 화면이 안 보는 카드 다섯이 함께 움직인다
 * (DESIGN §6.67이 그 판정을 적는다). ⚠️ **옛 비교 대상 `components/ui/card.tsx`는 2026-09-20에
 * 마지막 소비자와 함께 사라졌다** — 이 줄이 없는 파일을 근거로 들고 있었다.
 *
