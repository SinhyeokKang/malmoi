...
/**
 * 본문 상한 — **플랫폼 상한 이하의 값**이다. Vercel 문서는 함수 요청 본문 상한을 "4.5 MB"로만 적고 10진·2진
 * 어느 쪽인지 확인하지 않았다(4,500,000 또는 4,718,592) — 이 값은 두 해석 모두의 이하라, 2진이라면 그 사이의
 * 페이로드는 여기서 새로 걸린다. 플랫폼 방어를 우리 코드의 성질로 옮겨 두는 것이 요지다(`ALLOWED_HOSTS`와 같은 이유).
 * ⚠️ 올리려면 플랫폼 상한부터 실측한다.
 */
