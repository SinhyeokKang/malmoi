...
describe("createProject — 재검증한 값만 저장한다 (ARCHITECTURE §3.1)", () => {
   ...
   * 삭제가 비범위라 사용자가 슬롯을 되찾을 수 없다. `createInvitations`·`changeMember`가 프로젝트
...
describe("repository import Actions", () => {
  beforeEach(() => {
    ...
    hoisted.runRepositoryImportFromReader.mockResolvedValue({ ok: true, remainingEdits: 0, surfaces: [{ surfaceSlug: "default", status: "imported", count: 2, failed: 0, reason: null, errors: [] }] });
    hoisted.loadOpenPrUrl.mockResolvedValue(null);
  });
  it("OWNER만 실행하고 원결과를 반환한다", async () => {
    expect(await runRepositoryImport({ slug: "acme", approval: null })).toMatchObject({ ok: true, surfaces: [{ count: 2 }] });
    expect(hoisted.runRepositoryImportFromReader).toHaveBeenCalledWith(db.prisma, expect.objectContaining({ projectId: "p1", userId: OWNER, approval: null }), expect.any(Function));
    expect(hoisted.revalidatePath).toHaveBeenCalledWith("/projects/acme", "layout");
    expect(hoisted.revalidatePath).toHaveBeenCalledWith("/projects");
  });
  it("EDITOR·비로그인은 저장과 GitHub 호출 전에 거부한다", async () => {
    hoisted.session = sessionFor(EDITOR);
    expect(await runRepositoryImport({ slug: "acme", approval: null })).toEqual({ ok: false, error: "forbidden" });
    expect(await checkOpenPullRequest({ slug: "acme" })).toBeUndefined();
    hoisted.session = null;
    expect(await runRepositoryImport({ slug: "acme", approval: null })).toEqual({ ok: false, error: "unauthorized" });
  ...
  it("공용 읽기 실패를 그대로 반환하고 finally에서 캐시를 지운다", async () => {
    ...
    expect(await runRepositoryImport({ slug: "acme", approval: null })).toEqual({ ok: false, error: "tree-truncated" });
    expect(hoisted.revalidatePath).toHaveBeenCalledWith("/projects/acme", "layout");
  });
  it("DB 오류는 직렬화 경계에서 감추고 캐시를 지운다", async () => {
    ...
    expect(await runRepositoryImport({ slug: "acme", approval: null })).toEqual({ ok: false, error: "ingest-failed" });
    expect(hoisted.revalidatePath).toHaveBeenCalledWith("/projects");
  });
