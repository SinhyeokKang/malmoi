...
const mocks = vi.hoisted(() => ({ guard: vi.fn(), surfaceGuard: vi.fn(), read: vi.fn(), redirect: vi.fn((url: string) => { throw new Error(url); }), screen: vi.fn() }));
...
beforeEach(() => { vi.clearAllMocks(); mocks.guard.mockResolvedValue({ projectId: "p", role: "OWNER", archived: false }); mocks.surfaceGuard.mockResolvedValue({ projectId: "p", role: "OWNER", archived: false }); mocks.read.mockResolvedValue({ installed: true, sources: [] }); });
