...
/**
 * 멤버 화면의 조회 둘 (6b-2, DESIGN §6.65). **같은 축이다** — 이 표는 프로젝트 안의 전원이 보므로
...
describe("loadProjectList", () => {
  ...
  /**
   * **집계가 행까지 닿는다** (DESIGN §6.63). 조회가 배선됐다는 것과 그 값이 행에 실린다는 것은
  ...
  /**
   * **원격 신호가 행까지 닿는다** (DESIGN §6.63). 주입한 로더가 받는 입력도 함께 본다 —
