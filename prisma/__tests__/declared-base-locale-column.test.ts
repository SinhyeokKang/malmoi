...
function surfaceBlock(): string {
  const lines = SCHEMA.split("\n");
  const start = lines.findIndex((l) => l.startsWith("model TranslationSurface {"));
  if (start === -1) throw new Error("model TranslationSurface 블록이 schema.prisma에 없다");
  const end = lines.findIndex((l, i) => i > start && l.startsWith("}"));
  if (end === -1) throw new Error("model TranslationSurface 블록이 닫히지 않았다");
...
function docCommentAbove(block: string, field: RegExp): string {
  ...
  if (at === -1) throw new Error(`필드 ${field}가 TranslationSurface 블록에 없다`);
  const out: string[] = [];
  for (let i = at - 1; i >= 0 && lines[i]?.trimStart().startsWith("///"); i -= 1) out.unshift(lines[i] ?? "");
  return out.join("\n");
}

describe("schema.prisma — TranslationSurface.declaredBaseLocale", () => {
  /** additive여야 `/db`가 한 파이프라인 안에서 마이그레이션할 수 있다 (`/ship` 7단계). */
  it("nullable String이고 제약이 붙지 않는다", () => {
    expect(surfaceBlock()).toMatch(/^\s*declaredBaseLocale\s+String\?\s*$/m);
  });

  it("`baseLocale`은 그대로 nullable String이다 — 선언을 그 컬럼으로 접지 않았다", () => {
    expect(surfaceBlock()).toMatch(/^\s*baseLocale\s+String\?\s*$/m);
  });

  it("주석이 pull은 이 컬럼을 읽지 않는다는 사실을 든다 — 그것이 이 컬럼의 존재 이유다", () => {
    const above = docCommentAbove(surfaceBlock(), /^\s*declaredBaseLocale\s+String\?/);
