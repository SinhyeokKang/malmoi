...
- ⚠️ **로케일이 딱 하나인 리포도 하나 필요하다 — `i18n-single-locale`이다** (2026-09-24, launch-readiness L0.6 · **fork가 아니라 새 private 리포**, 파일 둘: `src/locales/en.json` + README). 로케일이 하나뿐인 리포는 붙일 수 없게 정했는데(ARCHITECTURE §3.1 "하나뿐이면 우연일 수 있다"), **설치된 나머지가 0·3·59로케일뿐이라** 그 거부 문구(`no-candidates` — "add a file for a second language")를 브라우저로 밟을 리포가 없었다. `pnpm ingest`로 거부를 확인했다. ⚠️ **둘째 로케일 파일을 넣지 않는다** — 넣으면 이 갈래를 밟을 유일한 리포가 사라진다. fork가 아닌 이유: 원본에 흔적이 남을 일이 없고 트리가 파일 둘이라 조회가 즉시 끝난다.
