...
말모이(`malmoi`): 로컬라이제이션 관리 도구(TMS). **이름은 1910년대 조선어사전 편찬 사업에서 왔다** — 흩어진 말을 여러 사람이 모아 하나로 만드는 일이 이 도구가 하는 일이다. 표기는 문서 본문 `말모이`, 화면 `Malmoi`, 코드·리포명·slug·도메인 `malmoi`.
