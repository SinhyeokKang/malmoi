 ...
 * 처음 들인 세트는 출처·라이선스를 모르는 Figma 커뮤니티 파일이라 교체했다. **목록은 교체 전
 * 253개 그대로이고** 원본의 265개 중 여기 든 것만 복사한다 — 원본에만 있는 12개(`bq-*`·`es-ct`·
 * `gb-eng`류·`ic`·`xa`·`xc`·`xo`)는 들이지 않았다. ⚠️ **"ISO 정식 배정 코드만"이 기준이 아니다** —
 * `ac`·`ta`·`eu`·`xk`도 정식 배정이 아닌데 로케일 하위태그로 오므로(`sq-XK`) 목록에 있다.
 */
