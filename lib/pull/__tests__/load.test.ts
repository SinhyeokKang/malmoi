...
it("로케일·값·완료 기준을 repeatable-read 스냅샷으로 읽는다", async () => {
  ...
  // 1층 판정값은 토큰 술어로 센다(T8) — 시각·저자로 세면 같은 밀리초 재저장과 전달 확인을 못 가른다.
  expect(state.unpublished).toBe(2);
  expect(tx.translation.count).toHaveBeenCalledWith({
    where: expect.objectContaining({ projectId: "p1", pendingEditToken: { not: null }, surface: { archivedAt: null } }),
  });
  expect(tx.translation.count.mock.calls[0]?.[0]).not.toHaveProperty("where.updatedAt");
