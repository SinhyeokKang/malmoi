...
/**
 * **sync 브랜치 이름의 생산자와 소비자가 같은가.** `syncBranchFor`가 `l10n/sync-<slug>`를 내는데 composite
 * action의 "열린 PR 경고"는 `--head l10n/sync`를 조회해 항상 "없음"을 찍었다 — 편집 손실 창의 유일한 신호가
...
describe("sync 브랜치 소비자 — l10n/sync-<slug>", () => {
  ...
  it("스모크가 생성 함수로 ref 이름을 만든다", () => {
    ...
    expect(smoke).not.toContain('"heads/l10n/sync"');
  });
