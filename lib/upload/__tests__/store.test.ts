...
it("공개 저장은 키를 바꾸지 않고 토큰과 판정된 MIME을 넘긴다", async () => {
  ...
  expect(sdk.put).toHaveBeenCalledWith("avatars/u/n.png", Buffer.from(bytes), expect.objectContaining({ access: "public", addRandomSuffix: false, contentType: "image/png", token: "test-token" }));
