...
it("공통 정리는 보안 접두 유무에 관계없이 두 왕복의 쿠키를 모두 만료시킨다", async () => {
  ...
  for (const name of ["malmoi-session-revocation", "__Host-malmoi-session-revocation", "malmoi-revocation-state", "__Secure-malmoi-revocation-state", "malmoi-login-link", "__Host-malmoi-login-link", "malmoi-link-state", "__Secure-malmoi-link-state"]) {
