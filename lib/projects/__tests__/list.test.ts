...
describe("projectStatus", () => {
  const READY = { archivedAt: null, installationId: "1", lastCommitSha: "a".repeat(40) };
  ...
  it("보관이 readiness를 이긴다", () => {
    ...
    expect(projectStatus({ archivedAt: new Date(), installationId: null, lastCommitSha: null })).toBe("archived");
  });
 ...
 */
describe("상태 문구", () => {
  const ALL = ["active", "archived", "setup", "awaiting_first_sync"] as const satisfies readonly ProjectStatus[];
  ...
  it("넷이 서로 다르다 — 한 문구로 접히면 상태를 구별할 수 없다", () => {
